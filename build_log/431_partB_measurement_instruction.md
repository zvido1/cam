...
**Date:** 2026-07-19
**Type:** PREREGISTRATION PACKAGE BUILD INSTRUCTION for Claude Code (Part B of 2). **v3.2.1 — RATIFIED 2026-07-19 by Tzvi — authorizes Stage 1 REBUILD only, zero model calls. Live execution remains unauthorized pending review, hashing, and separate sanction of the rebuilt artifacts.** v3.2 amends the ratified v3.1: §4 `tenant_share.basis_match` changed from exact-equality to INCLUSION (scoped to `tenant_share` only — `building_share` unchanged). **v3.2.1 (this state, committed after `af0dfcc`) changes NO rule** — it only corrects the amendment note's error-profile accounting and adds the §9.2 anticipated-divergence entry; the ratified §4 rule is byte-identical to `af0dfcc`. The prior Stage-1 artifacts (`4386d95`) and sanction token `9e7a2d1c…` are VOID for execution and are regenerated by the rebuild.
...
- **`tenant_share`**: `basis_match = match iff "operating_expenses" is a member of the grounded charge_basis_components set` (inclusion, not exact equality). A set lacking operating_expenses is `mismatch`; a set that is `unclear`/disputed/grounding-invalid is `undeterminable` (never false, never a terminal negative). **Additional grounded components (e.g. taxes, CAM) are preserved as part of the candidate's semantic identity and do NOT independently cause mismatch** — a single combined percentage that applies to operating expenses among other charge bases still applies to operating expenses. *Rationale (general, fixture-independent):* the ratified parameter is defined as "Tenant's Share of Operating **Expenses** percentage" (`PARAMETER_TARGETS`, `cam/adapters/lease_review/lease_parameter_block.py`); the definition requires the percentage to apply to operating expenses, and does NOT state it must apply *only* to operating expenses. Exact equality would add an exclusivity condition the definition never states. `charge_scope`: recorded metadata only — **does NOT gate qualification in v1** (explicit, not silently either-way). `text_role_ok = (text_role == operative_term)`. `value_ok = (value_completeness == self_contained AND value_token_present)`. **`applicability_match`**: encoded as a deterministic predicate over the candidate's basis/scope/support fields (a candidate whose basis/scope/support fit the share concept — the exact field predicate is written in the profile JSON, e.g. `charge_basis_components is non-empty AND resolves AND support_ok`), independent of value.
...
- **cand_04 under the inclusion rule (v3.2) — anticipated divergence, must be recorded not discovered:** the inclusion `basis_match` (v3.2 §4) matches iff `operating_expenses ∈ grounded components`. Atlas §3.3 defines CAM Charges as "all costs and expenses incurred by Landlord in operating, maintaining, repairing, and managing the Common Areas" — language from which a panelist could ground `operating_expenses` into cand_04's component set. **If that grounding occurs, cand_04's `basis_match` comes out `match` and the parameter may certify `satisfied` rather than `review_needed`.** Report which components each panelist grounded for cand_04, from which citations (candidate vs context), and whether any grounded `operating_expenses` from the CAM definition specifically. A `satisfied` cand_04 under this path is a **finding about panel grounding on an ambiguous CAM definition**, NOT a §9.1 mechanism failure (the mechanism worked: grounded, cited, same-candidate) and NOT a validation that Atlas contains a genuine opex share. Record it as such; do not read it as either.
...
**v3.2.1 RATIFIED 2026-07-19 — authorizes Stage 1 REBUILD only, zero model calls. Live execution remains unauthorized pending review, hashing, and separate sanction of the rebuilt artifacts.** Ratifying this document authorizes Claude Code to regenerate and commit the Stage-1 preregistration-package artifacts listed in §1 and §12, and nothing else. It does NOT authorize: the live measurement; any API/model call; changes under `cam/`; wiring; Gate B changes; prompt or configuration edits after Stage-1 review; patent drafting. The ~105-call run remains separately gated behind adversarial review and explicit sanction of the freshly hashed Stage-1 artifacts. The prior sanction token `9e7a2d1c…` is VOID. Wiring stays blocked behind Gates A–D (423 §8) and the two 430 blockers.

---

*Part B v3.2.1 — RATIFIED 2026-07-19 by Tzvi. Authorizes Stage 1 REBUILD only, zero model calls; live execution remains unauthorized pending review, hashing, and separate sanction of the rebuilt artifacts. v3.2 amended v3.1 at §4 (`tenant_share.basis_match` → inclusion; `building_share` unchanged); v3.2.1 changed no rule (corrected amendment note + §9.2 anticipated-divergence entry; §4 byte-identical to af0dfcc). Prior Stage-1 build `4386d95` and token `9e7a2d1c…` VOID for execution; provenance retained. Preregistration package build instruction for governed evidence-selection measurement. Five aligned parameter-series per lease/parameter (no cross-series pooling); validation split across sidecar and repo/manifest/report; canonical target 105 primary calls (90 if cand_06 excluded). Next review target: the rebuilt Stage-1 artifact package (blind leak review, fresh conversation), especially 431_selector_prompt.txt and 431_requirement_profiles.json.*
