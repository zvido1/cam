...
**Amendment Record — v3.1 → v3.2 (2026-07-19):** §4 `tenant_share.basis_match` changed `equals exactly {operating_expenses}` → `operating_expenses ∈ components` (inclusion). Reason: the ratified parameter definition ("Tenant's Share of Operating **Expenses** percentage", `PARAMETER_TARGETS`) requires the percentage to apply to operating expenses but does not require it to apply *only* to operating expenses; exact equality added an unstated exclusivity condition. *Measurement-scope note (kept here, NOT in the fixture-independent profile):* both rules behave identically on the two seeded leases — cand_01 matches and cand_04 mismatches under either — because neither fixture contains a blended operating_expenses+taxes+CAM share; the exact-vs-inclusion distinction bears only on a blended-base candidate not present in this measurement. The change removes a future false negative for a combined-base mechanism; it does not weaken the Atlas test (a grounded {taxes, CAM} still mismatches). Raised by GPT adversarial review of the Stage-1 profiles; decided by Tzvi. [v3.1 lineage retained below for provenance.]
