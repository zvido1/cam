...
**Date:** 2026-07-19
**Type:** PREREGISTRATION PACKAGE BUILD INSTRUCTION for Claude Code (Part B of 2). **v3.2 — RATIFIED 2026-07-19 by Tzvi — authorizes Stage 1 REBUILD only, zero model calls. Live execution remains unauthorized pending review, hashing, and separate sanction of the rebuilt artifacts.** v3.2 amends the ratified v3.1: §4 `tenant_share.basis_match` changed from exact-equality to INCLUSION (see Amendment Record below; the amendment is scoped to `tenant_share` only — `building_share` is unchanged). The prior Stage-1 artifacts (`4386d95`) and sanction token `9e7a2d1c…` are VOID for execution and are regenerated by the rebuild.
