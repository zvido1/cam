...
**Date:** 2026-07-19
**Type:** PREREGISTRATION PACKAGE BUILD INSTRUCTION for Claude Code (Part B of 2). **v3.3 — RATIFIED 2026-07-19 by Tzvi — authorizes Stage 1 REBUILD only, zero model calls. Live execution remains unauthorized pending the rebuilt artifacts, both fresh blind reviews, the informed audit, and a separate explicit sanction.** (Ratification attaches to the corrected v3.3 text: §5 — sibling-charge false matching is blocked at the policy layer when the classification conforms to the relation-bearing contract, semantic model error remains possible; §9.2 — a satisfied `cand_04` is the panel asserting, with resolving value-to-basis citations, that the linkage holds, NOT proof the linkage is semantically or legally correct.) v3.3 repairs a blocking contract defect found by blind Review B and confirmed by GPT's informed audit: the shared semantic field `charge_basis_components` recorded *which bases were present/co-mentioned*, not *which bases the candidate's own value applies to* — so a tax percentage co-located with an operating-expense clause could be grounded `[operating_expenses, taxes]` and falsely certified as the opex share (a lossy representation laundering a relationship error into a match, uncatchable by any §9.2 observation because the panel could classify every fact honestly and the policy would still certify the wrong value). The prior Stage-1 package (`5954e6e`) and sanction token `ccf03284…` (and all earlier tokens) are VOID for execution.

**Amendment Record — v3.2.1 → v3.3 (2026-07-19):** Four substantive changes. (1) The shared semantic field `charge_basis_components` is replaced schema-wide by the relation-bearing `value_applies_to_charge_basis_components`, defined as *the charge bases directly governed by the candidate's own value expression, as established by cited text linking that value to those bases* — excluding charge categories merely mentioned in context and categories governed by a different value expression. (2) `field_support` for this field must cite text establishing the value-to-basis relationship, not separate citations proving that the value and the charge category each occur somewhere; unresolved linkage routes to `undeterminable`. (3) `tenant_share.basis_match` inclusion (`operating_expenses ∈ set`) now operates on the new relation-bearing field. (4) The §5 uncertainty wording is rebalanced (removes "Guessing is worse than abstaining"; makes determinate and uncertainty outputs equally un-preferred; keeps the bounded-excerpt prohibition) — closing the abstention lean blind Review A flagged. **Scope:** the field rename is schema-wide because it repairs the shared field's meaning; it is NOT permission to alter `building_share`'s matching rule, which stays exact-match, unseeded, and recorded as unresolved amendment debt (GPT informed-audit B1 ruling). **§9.2 consequence:** after v3.3, a co-occurrence false match is no longer an acceptable "observation" — any cand_04 result is interpretable ONLY because the instrument now carries the value-to-basis relationship explicitly. Raised by blind Review B (defect) and GPT informed audit (blocking ruling + minimal fix); decided by Tzvi. [v3.2/v3.2.1 lineage retained below for provenance.]

**Amendment Record — v3.1 → v3.2 (2026-07-19):** §4 `tenant_share.basis_match` changed `equals exactly {operating_expenses}` → `operating_expenses ∈ components` (inclusion). Reason: the ratified parameter definition ("Tenant's Share of Operating **Expenses** percentage", `PARAMETER_TARGETS`) requires the percentage to apply to operating expenses but does not require it to apply *only* to operating expenses; exact equality added an unstated exclusivity condition. *Error-profile note (corrected — the rules are NOT simply equivalent):* the two rules behave identically on the seeded documents' actual **content** (Atlas contains "Operating Expenses" zero times, verified 430 §3), but they operate on what the **panel returns**, which is not the same thing. Concretely: Atlas §3.3 defines CAM Charges as "all costs and expenses incurred by Landlord in operating, maintaining, repairing, and managing the Common Areas." A panelist could plausibly ground `charge_basis_components` as `{operating_expenses, CAM, taxes}` from that sentence (the words "operating", "costs and expenses" are present). Under exact-equality cand_04 mismatches regardless of grounding; under inclusion, that particular grounding makes cand_04 **match**, and the Atlas concept-substitution case could come out `satisfied` rather than `review_needed`. So the change does NOT strictly dominate exact-equality — it trades a false-negative exposure (a genuine blended-base opex share wrongly rejected) for a false-positive exposure (a tax/CAM share accepted because "operating" appears in a CAM definition). This is decided in favor of inclusion because it follows the ratified definition, NOT because it is uniformly safer. **The divergence, if it occurs, is a §9.2 OUTCOME OBSERVATION, not a §9.1 failure:** the mechanism still worked (grounded, cited, disagreement-preserved, same-candidate); it must be ANTICIPATED in the report so a `satisfied` cand_04 is read as a finding about panel grounding on an ambiguous CAM definition, not as a malfunction and not as a validation that Atlas is fine. Raised by GPT (rule) and Claude Code (error-profile correction); decided by Tzvi. [v3.1 lineage retained below for provenance.]
...
- **`tenant_share`**: `basis_match = match iff "operating_expenses" is a member of the grounded value_applies_to_charge_basis_components set` (inclusion, not exact equality). A set lacking operating_expenses is `mismatch`; a set that is `unclear`/disputed/grounding-invalid, OR whose value-to-basis linkage is not citation-grounded, is `undeterminable` (never false, never a terminal negative). **Additional grounded components (e.g. taxes, CAM) are preserved as part of the candidate's semantic identity and do NOT independently cause mismatch** — a single combined percentage that applies to operating expenses among other charge bases still applies to operating expenses. **The field is relation-bearing (v3.3):** `value_applies_to_charge_basis_components` contains ONLY the bases directly governed by *this candidate's own value expression*, established by cited text linking that value to those bases — NOT charge categories merely mentioned in context, and NOT bases governed by a different value expression. A tax percentage co-located with an operating-expense clause does not acquire `operating_expenses`; a percentage whose value-to-basis linkage is not grounded routes to `undeterminable`, never `match`. *Rationale (general, fixture-independent):* the ratified parameter is defined as "Tenant's Share of Operating **Expenses** percentage" (`PARAMETER_TARGETS`, `cam/adapters/lease_review/lease_parameter_block.py`); the definition requires the *percentage* to apply to operating expenses, and does NOT state it must apply *only* to operating expenses. Membership-in-a-co-occurrence-set is not application; the relation-bearing field is what makes "applies to operating expenses" a checkable claim rather than a co-location artifact. `charge_scope`: recorded metadata only — **does NOT gate qualification in v1** (explicit, not silently either-way). `text_role_ok = (text_role == operative_term)`. `value_ok = (value_completeness == self_contained AND value_token_present)`. **`applicability_match`**: encoded as a deterministic predicate over the candidate's basis/scope/support fields (a candidate whose basis/scope/support fit the share concept — the exact field predicate is written in the profile JSON, e.g. `value_applies_to_charge_basis_components is non-empty AND resolves AND support_ok`), independent of value.
- **`building_share`**: not seeded (§6); profile declared but unexercised. Its `basis_match` stays exact-equality `{operating_expenses}` over the renamed relation-bearing field — **the field rename applies (shared field), but the matching RULE is unchanged** (GPT informed-audit B1 ruling: fixing the tenant_share/building_share asymmetry would broaden the amendment into an unmeasured parameter; recorded as unresolved amendment debt, resolved before building_share is ever seeded, not before these calls).
- **`base_rent`**: `value_applies_to_charge_basis_components` = `not_applicable` (Part A §4.1; charge basis is not a meaningful attribute of base rent). `text_role_ok = (text_role == operative_term)`. `value_ok = (value_completeness == self_contained AND value_token_present)`. A `definition`/`cross_reference_only` candidate is `not_qualified` by the value_ok predicate.
- **`rent_adjustment_pct`**: `value_applies_to_charge_basis_components` = `not_applicable`. `text_role_ok = (text_role == operative_term)` (a `narrative` role fails this predicate — the case-4 discriminator, encoded as a rule, not an expected outcome). `value_ok = (value_completeness == self_contained AND value_token_present)`.
...
Each panelist, per (candidate + envelope) × parameter, returns the Part A §4.1 contract: `parameter_family_relevance` (`relevant`|`not_relevant`|`unclear`), `candidate_support_state` (`supports_mechanism`|`does_not_support_mechanism`|`insufficient_context`|`unclear`), `text_role`, `value_completeness`, parameter-type-specific **`value_applies_to_charge_basis_components`** (the relation-bearing field, v3.3 — closed enums + `other`/free-text)/`charge_scope`, `candidate_citations[]`, `context_citations[]` (id + verbatim quote), **`field_support`** (citation ids per field — for `value_applies_to_charge_basis_components`, the cited text must establish the *value-to-basis relationship*, not merely that the value and the charge category each occur somewhere), `reason`, `confidence`.
...
- `value_applies_to_charge_basis_components` read from cited text that links THIS candidate's own value to those bases; a basis merely co-mentioned in context, or a basis governed by a different value expression, is NOT included; uncited or unlinked basis → `unclear`/`undeterminable`.
- `does_not_support_mechanism` and `insufficient_context` are first-class permitted answers (Part A §4.3); the panelist may not infer document-level absence from bounded context. **Uncertainty and determinate outputs are equally un-preferred (v3.3):** the prompt asks for a determinate classification when the supplied text grounds one, and `insufficient_context`/`unclear` only when it does not — with no extra encouragement toward either, and NO wording (such as "guessing is worse than abstaining") that tilts the distribution toward abstention. The bounded-excerpt prohibition (never infer document-level absence) is retained; it is a correctness rule, not an abstention preference.
...
**Relation-bearing field — build-time deterministic tests (v3.3, required Stage-1 artifacts).** Because the `charge_basis_components` → `value_applies_to_charge_basis_components` change blocks a sibling-charge false match **at the policy layer when the classification conforms to the relation-bearing contract** (it does NOT make model semantic error impossible — a panelist can still assert, with resolving but semantically inadequate citations, that a tax value applies to operating expenses; the contract makes that claim representable and policy-checkable, not metaphysically unavailable), the harness must ship deterministic unit tests (zero model calls, run at build) that assert the profile+validator handle these four synthetic classifications correctly — these test the RULE against constructed inputs, they are NOT fixtures and carry no lease text:
1. one combined percentage grounded as applying to operating_expenses + taxes → `value_applies_to_charge_basis_components = [operating_expenses, taxes]` → `tenant_share.basis_match = match` (inclusion; opex is governed by the value).
2. separate operating-expense and tax percentages in one context, the tax candidate under test → that candidate's field = `[taxes]` → `basis_match = mismatch` (opex is a *sibling* value's basis, not this value's).
3. a tax percentage with an operating-expense clause merely nearby (co-mention, no value-to-basis linkage to opex) → field = `[taxes]`, opex NOT added → `basis_match = mismatch`.
4. a percentage whose value-to-basis linkage is not citation-grounded → field routes to `undeterminable` → `basis_match = undeterminable` (never `match`, never a terminal negative).
These four tests passing at build is a Stage-1 gate; failure halts the build (the relation-bearing contract is not correctly enforced).
...
- Observed `value_applies_to_charge_basis_components` for cand_04 (22.4%); whether `basis_match=mismatch` vs the declared opex requirement.
- **cand_04 under the relation-bearing field (v3.3) — the v3.2.1 "co-occurrence observation" reading is now RETIRED:** before v3.3 this entry said that if a panelist grounded `operating_expenses` anywhere in cand_04's context the inclusion rule would match, and a `satisfied` cand_04 should be recorded as "a finding about panel grounding on an ambiguous CAM definition." **That reading is no longer valid and must NOT be used.** It rested on the broken contract in which the basis field recorded co-occurrence; under that contract a `satisfied` result could be a lossy-representation artifact (the panel classifies every fact honestly and the policy still certifies the wrong value), and calling it an "observation" would be reporting an instrument defect as model behavior. Under v3.3, `value_applies_to_charge_basis_components` carries only the bases the candidate's OWN value applies to, established by cited value-to-basis linkage. Therefore: **a `satisfied` cand_04 is now interpretable as the panel asserting, with resolving value-to-basis citations, that the 22.4% value itself applies to operating expenses** — not merely that operating expenses are mentioned near it. It remains an observed panel judgment, NOT proof that the asserted linkage is semantically or legally correct (the panel may cite resolving but semantically inadequate text; the mechanism governs whether the claim is grounded and cited, not whether it is true — mechanism success stays answer-key-independent). Report which components each panelist grounded for cand_04, the citations establishing the value-to-basis linkage (candidate vs context), and whether the linkage was grounded or routed to `undeterminable`. A `satisfied` cand_04 is NOT a validation that Atlas contains a genuine opex share; if the linkage is absent, the correct result is `undeterminable`→`review_needed`, never a co-occurrence match.
...
**v3.3 RATIFIED 2026-07-19 by Tzvi — authorizes Stage 1 REBUILD only, zero model calls. Live execution remains unauthorized pending the rebuilt artifacts, both fresh blind reviews, the informed audit, and a separate explicit sanction.** Ratifying this document authorizes Claude Code to regenerate and commit the Stage-1 preregistration-package artifacts listed in §1 and §12 — Stage 1 REBUILD only, zero model calls — and nothing else. It does NOT authorize: the live measurement; any API/model call; changes under `cam/`; wiring; Gate B changes; prompt or configuration edits after Stage-1 review; patent drafting. The ~105-call run remains separately gated behind both blind reviews (prompt + profiles, fresh conversations), GPT's informed audit, and explicit sanction of the freshly hashed Stage-1 artifacts. The prior Stage-1 package `5954e6e` and sanction token `ccf03284…` (and all earlier tokens `9e7a2d1c…`, `5ed0c5cc…`, `ecbf512d…`, `0ebb93ba…`) are VOID for execution. Wiring stays blocked behind Gates A–D (423 §8) and the two 430 blockers.

---

*Part B v3.3 — RATIFIED 2026-07-19 by Tzvi. Authorizes Stage 1 REBUILD only, zero model calls; live execution remains separately gated behind the rebuilt artifacts, two fresh blind reviews, the informed audit, and a separate explicit sanction. v3.3 repairs a blocking contract defect (blind Review B + GPT informed audit): the shared basis field `charge_basis_components` → relation-bearing `value_applies_to_charge_basis_components` (only bases the candidate's own value applies to, cited value-to-basis linkage required); `tenant_share.basis_match` inclusion now operates on the relation-bearing field; §5 abstention wording rebalanced (Review A); §9.2 co-occurrence-observation reading RETIRED. Doctrine preserved: sibling-charge false matching is blocked at the policy layer when the classification conforms to the contract (not made impossible); a satisfied cand_04 is a grounded panel assertion, not proof of semantic/legal correctness. `building_share` rule unchanged (exact-match, unseeded, amendment debt — B1 ruling). Four build-time relationship tests required (§5). Prior package `5954e6e` and token `ccf03284…` VOID for execution; provenance retained. Lineage: v3 `ce2159b` → v3.1 `8755119`/`e6c07ab` → v3.2 `af0dfcc` → v3.2.1 `61d03d1`/`fa61660` → v3.3 (this state). Canonical target 105 primary calls (90 if cand_06 excluded). Next review targets after rebuild: 431_selector_prompt.txt (prompt changed — blind Review A re-run) and 431_requirement_profiles.json + 431_output_schema.json (field changed — blind Review B re-run), then informed audit.*
