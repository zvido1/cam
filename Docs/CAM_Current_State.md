...
>
> **SALVAGE POSITION.** Seven of the nine §9.1 criteria are computable from the immutable sidecar with **zero further model calls** (`build_log/431_postrun_partial_validation.json`, an explicitly post-run artifact that is **not** the preregistered validator and **may not be labelled §9.1**). #6 is not computable — its data was never recorded, so no post-hoc computation can supply it. #3 is blocked on a definition, not on data. **A complete §9.1 requires a corrected package and a re-run.**
>
