# CAM Current State — Updated 2026-07-21 (431 STAGE-1 SANCTIONED — governed evidence-selection measurement preregistered, frozen, reviewed, and cleared to run; NOT YET RUN.)

> **=== LATEST STATUS 2026-07-21 (read THIS FIRST — Step 431: governed evidence-selection measurement. Stage-1 preregistration package SANCTIONED for execution; the ~105-call Stage-2 run is authorized but has NOT been run. Zero model calls made in the entire 431 arc to date.) ===**
>
> **431 — Governed evidence-selection measurement (Part A + Part B; instruction `build_log/431_partB_measurement_instruction.md`, v3.3 RATIFIED; provenance `build_log/431_blind_review_provenance.md`).** A preregistered measurement of the governed evidence-SELECTION mechanism over seven provisioned candidates across two leases (Atreca known-good foil + Atlas forcing cases), run by a frozen three-model panel (A=claude-sonnet-4-6, B=gpt-5.5 at provider-default temp, C=grok-4.3). **Two-stage gate throughout: Stage 1 = build the preregistration package (zero model calls, call path structurally `NotImplementedError`); Stage 2 = the ~105 live calls, separately sanctioned.** The arc is entirely Chat-spec + Claude-Code-build; no calls have fired.
>
> **Frozen sanctioned package: commit `65556ee`. Sanction token: `47cb312a`.** Seven artifacts under `build_log/` (measurement_config, requirement_profiles, output_schema, selector_prompt, fixture_preflight, run_431_selection_measurement.py, config_manifest). Preflight 7/7 admitted; cand_06 unique at `[3619,3660)`; both lease hashes match 430. Call gate refuses all prior void tokens. All earlier packages (`4386d95`, `5954e6e`) and tokens (`9e7a2d1c`, `5ed0c5cc`, `ecbf512d`, `0ebb93ba`, `ccf03284`) are VOID for execution.
>
> **The load-bearing correction of the arc (v3.3): relation-bearing basis field.** The schema field `charge_basis_components` (which recorded *which bases were present/co-mentioned*) was replaced schema-wide by `value_applies_to_charge_basis_components` (*which bases the candidate's OWN value applies to*, established by cited value-to-basis linkage). The old field allowed a co-occurrence false match — a tax/CAM percentage co-located with an operating-expense clause could be grounded `[operating_expenses, taxes]` and falsely certified as the opex share, a lossy-representation error uncatchable by any observation because the panel could classify every fact honestly and the policy would still certify the wrong value. This was found by a BLIND profile review and confirmed BLOCKING by the informed audit; the fix moves the laundering error out of the mechanism, leaving honest panel semantic error (the thing the measurement observes). `tenant_share.basis_match` = inclusion (`operating_expenses ∈ set`) over the relation-bearing field. `building_share` rule unchanged (exact-match, unseeded — recorded amendment debt, do NOT fix before it is ever seeded).
>
> **Review chain (all closed):** (1) BLIND Review A (prompt/schema, run incognito, confirmed uncontaminated) — ZERO answer-key leaks; all findings response-shape (applicability-flag "establishes" presupposition; one-sided co-location caution; abstention lean; enum ordering) — none directional on the fixtures. (2) BLIND Review B (profiles, stripped of all finding/fix/fixture traces per the provenance note) — independently CONFIRMED the co-occurrence fix closed; flagged `basis_match` as a basis-screen-not-share-screen (inert on these seven fixtures, real generality/claim-scope debt), building_share asymmetry (recorded debt), unanimity gate (deliberate `431_certpolicy_v1` conservatism). (3) GPT INFORMED audit — **SANCTIONED** `65556ee`/`47cb312a`; ruled A1/A2 nonblocking on the fixtures (the v3.3 relation-bearing field + citation-grounding neutralizes the flag's presupposition; cand_01's opex link is syntactically self-contained so the co-location caution can't suppress it), B1 inert-here/generality-debt, B2/B3 prior rulings hold. (4) One verification qualification (relationship test (c)) — Claude Code confirmed FORM A (test (c)'s value genuinely applies to taxes with opex merely co-mentioned; the no-grounded-linkage case is test (d), correctly expecting undeterminable), read-only, zero bytes changed, token valid.
>
> **Four build-time relationship tests (deterministic build gate, zero calls):** (a) combined opex+taxes→match; (b) sibling tax candidate→mismatch; (c) co-mention-no-opex-linkage→mismatch; (d) ungrounded linkage→undeterminable. All pass; scope note (per Claude Code + GPT): (b)/(c) exercise the rule's EXCLUSION branch given a populated field; (d) tests citation presence/resolution — the model's semantic linkage judgment is by-design NOT code-covered (measured by the live run, not the unit tests).
>
> **CLAIM BOUND (must survive to the report and the patent record):** 431 is a measurement of GOVERNED SELECTION over these seven provisioned candidates — NOT general semantic accuracy, NOT recall, NOT production readiness, NOT share-vs-fee discrimination (B1 generality debt). A satisfied cand_04 (the concept-substitution forcing case) is a grounded panel assertion that the 22.4% value applies to opex, NOT proof it is legally correct and NOT validation that Atlas contains a genuine opex share; §9.2 pre-registers recording which components each panelist grounded and from which citations.
>
> **NEXT: Stage-2 execution** — Claude Code runs `run_431_selection_measurement.py` against `65556ee` under token `47cb312a`, ~105 primary calls (7 candidates × 5 canonical × 3 panelists), producing the sidecar + `431_runtime_seam_capture.json` + `431_validation.json` + `431_repository_seam_check.json` + report. This is the FIRST model-call activity of the arc. Authorized, not yet run — awaiting Tzvi's go. Any edit to the package voids the token and requires re-freeze + re-review.
>
> **=== END 431 CURRENT-FRONTIER BLOCK — historical status blocks below (428 and earlier) remain the record; the 431 arc supersedes 428/429 as the active frontier. Note: the 429 "robust target resolution" work described below is a DIFFERENT (parameter-extraction) track from 431's evidence-SELECTION measurement; both are live, 431 is the sanctioned one. ===**
