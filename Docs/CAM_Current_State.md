...
> ## STILL OPEN
>
> **1. PRECISION — the unasked question.** Everything above measures whether the right evidence *reaches* the panel and *survives the gate*. **Nothing measures whether the spans are the RIGHT evidence for those elements.** LP-27's spans resolve to **§5.1 and §11.2 only** — §11.2 is an indemnity clause, and whether it properly evidences *"right to monetary damages for landlord default"* was never tested. This is the mirror image of the LP-12 seven-spans observation in `FINDING_423_stack_verified_live.md`: **exclusive assignment loses evidence; non-exclusive assignment may flood it.** A false `explicitly_present` grounded in an irrelevant clause is the same class of defect as the false `missing` this arc set out to fix, and the current measurements would not detect it.
>
...
>
> **3. Two layering tests fail by design.** `test_423a_evidence_span_substrate.py::TestSeamStandaloneAndUninvasive::test_module_not_imported_by_live_stage5_pipeline_files` and `test_423c_element_guided_elicitation.py::TestPipelineSeam::test_no_live_pipeline_file_imports_elicitation_module`. Both `inspect.getsource` on `lease_adapter`, `lease_extract`, `lease_coverage` and assert `assertNotIn("lease_evidence_spans", src)` / `assertNotIn("lease_element_elicitation", src)`.
>
> **The distinction matters and should not be blurred: they assert that coverage must not import the 423 stack. That is a not-yet-connected precondition, NOT a direction constraint.** The 423A test's own docstring gives its purpose as proving *"no Stage 5 verdict behavior changed as a side effect of adding it"* — a non-interference guarantee for a slice that was only building the substrate, with source-text `assertNotIn` as the proxy. A real layering rule constrains direction: that the 423 stack must not import coverage. **The genuine direction checks still pass** — `test_evidence_spans_module_not_modified_by_this_slice` and `test_no_lp_taxonomy_leakage_into_span_resolution`. The seam respects the doctrine and violates only the precondition. **These two should be deliberately retired and replaced with "the seam, and only the seam, imports this" — not made green.** Suite otherwise 350 passed.
>
> **4. `span_evidence_records` is dead, and two comments are stale.** Assigned at `lease_coverage.py:211` and `:222`, **read nowhere.** The span records — offsets, `elicited_by`, `verification_status`, the `section_ref` the locator wrote back — are discarded at the seam. **Span provenance evaporates: nothing downstream can audit which spans produced a verdict**, which is precisely what open item 1 would need. Stale comments committed as-is: the header still says `EXPERIMENT, LP-07 ONLY` and `that one edit is the whole rollback`; the seam comment says `423 seam (LP-07 only, SPAN_EVIDENCE_LPS)`. The LP set is two and rollback is now a file checkout.
>
> **5. Whether to extend the seam to the remaining 31 LPs, and at what cost.** Measured `api_calls_total`: **no seam 92; one seamed LP 93/93/94; two seamed LPs 94/94** on a clean panel (91/91 on the degraded panel, where fallbacks alter the count). That is **~+1 call per seamed LP**, consistent with one elicitation call per LP, ±1 run-to-run. Extrapolating: all 33 LPs would be roughly +33 calls on ~92, a ~36% increase — material but not prohibitive. **The blocker on extending is open item 1, not cost:** seaming 31 more LPs without a precision measurement multiplies an unmeasured risk 31-fold.
>
