# CAM Current State — Updated 2026-08-23 (THE SEAM IS BUILT, MEASURED AND COMMITTED. The 08-20 block's operative conclusion — "the architecture exists and is unwired" — is superseded: 423C span elicitation is now the evidence source for LP-07 and LP-27, the false LP-07 finding is fixed, and a second defect found en route (span evidence was uncitable) is fixed too. NOT deployed. Precision unasked.)

> **ACTIVE PROJECT REGIME — 2026-08-23 THROUGH 2026-11-11.** The current operating plan is `Docs/CAM_Patent_Go_No_Go_Plan_2026.md`. Until the November 11 patent investment decision, discretionary engineering is paused. Engineering proceeds only when it answers a named decision question, supplies counsel evidence, enables a scheduled validation, repairs decision-critical evidence, or addresses material security/confidentiality exposure. The technical frontier below remains authoritative; it is not itself the active roadmap.

> **=== LATEST STATUS 2026-08-23 (read THIS FIRST — Steps 455–459, arc ran 2026-08-21 → 2026-08-23. The 2026-08-20 block below is superseded in its FIX DIRECTION section only; its diagnosis stands and is the reason this work exists.) ===**
>
> ## RESOLVED SINCE THE 08-20 BLOCK
>
> **1. The 423 stack is wired.** `lease_coverage.py` now sources evidence for the LPs in `SPAN_EVIDENCE_LPS` from 423C element-guided span elicitation instead of extraction's exclusive bucket. Committed **`134998b`**. `SPAN_EVIDENCE_ENABLED = True`, `SPAN_EVIDENCE_LPS = {"LP-07", "LP-27"}`. Every other LP keeps the extraction path unchanged; elicitation failure falls back to it and logs the fallback, so there is no silent substitution. **NOT deployed. NOT extended beyond that LP set.**
>
> The 08-20 block's census result — that `elicit_and_resolve_for_lp` and the rest of the stack had **no production caller at all** — is no longer true for `elicit_and_resolve_for_lp`, `dedupe_elicited_spans`, `build_canonical_source` and `load_expected_elements_by_lp`. The 423B entry points (`propose_spans`, `resolve_proposed_spans`, `build_span_universe_sidecar`) and 427's Gate B (`check_gate_b`, `enforce_gate_b`) **remain uncalled.**
>
> **2. The false LP-07 finding is fixed, and measured.** `"Tenant's proportionate share calculation method is defined"` moved from `elements_missing` to `elements_found` and **held 3/3 across the three LP-07-only seam runs**, then held again across two further two-LP runs — 5 of 5 seam runs total, with `22.4` present in the evidence every time.
>
> **The evidence got SMALLER and more correct.** Extraction's bucket delivered 2,636 characters that did not contain the definition; five verified spans deliver 1,565–1,957 characters that do. This is the point of the whole architecture stated as a measurement: exclusive assignment was not supplying too little text, it was supplying the wrong text.
>
> **3. A second defect was found and fixed en route — span evidence was uncitable.** The seam assembled verified clause bodies and stripped source-location metadata, deliberately, so that synthetic span markers would not change what evaluators read. That collided with the 305 contract, which requires a `section_ref` for any presence verdict. The result on LP-27: **0 found / 1 missing, with 8 of 10 elements suppressed as `citation_required_but_absent`** — evaluators A and C returning `explicitly_present` at `confidence=high`, consensus reached, and the verdict then replaced by deterministic code. **The failure occurred after correct semantic evaluation.** Extraction buckets retain section headings; span evidence is often mid-section fragments with nothing truthfully nameable.
>
> The **Step-455 locator prefix** resolved it. Each verified span is preceded by `[<Section N.N>]` derived from its canonical offsets by backward scan over a **line-anchored** heading index. Line anchoring is load-bearing: the same `Section N.N.` strings occur inline as cross-references, and an unanchored scan tags the Proportionate Share definition as `Section 8.3` when it lives in `Section 1.2`. On Atlas, 70 matches — 65 headings, 5 cross-references, all 5 excluded.
>
> **Result: LP-27 now 8 found / 1 missing, zero suppressions, across two clean-panel runs** (role B `gpt-5.5` 197/197, `is_fallback` False). LP-level `review_needed`/confidence `low` → **`partial`/confidence `high`**. `citation_required_but_absent` appears nowhere in either run; all ten merge reasons are `None`. Both `missing` elements were `missing` before the locator too — genuine absences, not casualties.
>
> **4. The evaluators now cite the locator they were given.** LP-07's `proportionate_share_calculation`, all three evaluators, both runs: **`'Section 1.2'`**, `citation_quality='section_and_quote'`. That is the section the definition genuinely lives in. The five prior runs produced `None`, `'Paragraph 1'`, `'Para. 1'`, `'LP-07 CAM provision, paragraph 1'`, `'Proportionate Share definition'` — **not one a section reference.** Before the locator, LP-07 cleared the citation gate on manufactured labels while LP-27's panel was penalised for honestly returning `None`; the gate was selecting for fabrication.
>
> Audited rather than eyeballed: the `[...]` prefixes actually emitted were parsed from each LP's assembled text and every citation checked against them. **81 of 82 non-null citations match a supplied locator.** The one exception is role B returning `'LP-22 Sections 19.1-19.3'` on the single element with no supporting span; it merged to `missing` and passed nothing through the gate. Manufacture is reduced, not eliminated.
>
> ## STILL OPEN
>
> **1. PRECISION — ASKED AND ANSWERED (Step 460). The seam is NOT vindicated wholesale: 6 of 8 "found" elements are clean, 2 are false positives.** Full per-element evidence, lawyer-readable, at `build_log/460_LP27_precision_evidence.md`.
>
> - **Element 6, "Tenant has right to monetary damages for landlord default" — FALSE PRESENT.** Five of six judgments across the two clean-panel runs rest on **Section 11.2, an indemnification clause**. No span before the panel grants Tenant damages for landlord default, because the lease contains no such grant. Merged `explicitly_present` in both runs. Indemnity against third-party claims is not a damages right; the word doing the work is *"damages"* inside a list of indemnified categories.
> - **Element 7, specific performance / injunctive relief — FALSE PRESENT, and weaker still.** It rests entirely on a 91-character savings clause: *"These rights are in addition to any other remedies available to Tenant at law or in equity."* A clause preserving **unspecified** remedies is being read as evidence that a **specific** remedy exists. The two runs disagree — `explicitly_present` vs `implicitly_present` — the instability you expect on thin evidence. Document-level check: **`"specific performance"` 0 hits, `"injuncti"` 0 hits, `"equitable relief"` 0 hits** in the lease. The elicitor did not miss better evidence; there is none to find.
> - **The diagnostic asymmetry, and it is the important part.** On element 4, evaluator B quoted *"Tenant shall have the right to draw upon the Security Deposit as a setoff against damages"* and voted **`missing` anyway**, because the element asks for offset against **rent** and the lease gives setoff against the **deposit**. A near-miss correctly rejected — same panel, same run — while a further miss was accepted at `explicitly_present` on element 6. **This is not general panel laxity.** What gets waved through is **generic, topically adjacent** evidence, and span elicitation surfaces more of it than buckets did **because it searches by description rather than by location**.
>
> The same savings clause (span 6) is the cited basis for four different elements — 6, 7, 9 and 10. For element 9 it is a direct hit and for 10 a fair inference correctly marked implicit; the exposure is concentrated where the clause is being asked to do work it does not do.
>
> Related: the LP-27 selection rationale was **withdrawn** in `455_code_status.md`. LP-27 was chosen on the belief its evidence was scattered across §§5.1/6.3/13.3/8.2/18.1/21.1 while its bucket held only §5.1. Both halves were wrong — the bucket already contained the landlord-default clause in full, and the span path returned only §5.1 and §11.2. LP-27 was never a case of destructive exclusive assignment; it turned out to be a good citability test by accident.
>
> **1B. SPAN ELICITATION UNDER-INCLUDES ACROSS SECTIONS — structural, and larger than the two false positives above.** This is the mirror of bucket assignment **over-including within** one.
>
> **Section 11.3, Limitation of Liability**, canonical offsets **[15490, 15748]**:
>
> > *Neither party shall be liable to the other for any consequential, indirect, punitive, or special damages arising under this Lease. Landlord's liability shall be limited to Landlord's interest in the Building and Land.*
>
> **Span 7 ends at 15251.** The panel was handed the indemnity and stopped **239 characters short** of the clause that limits it.
>
...
>
> **2. LP-12 extraction gate — now MEASURED, not inherited.** Earlier status files recorded the attribution as unverified because the harness truncated the exception at 90 characters. The untruncated message, captured 2026-08-23:
>
> > `Extraction completeness failure: 1 required LP(s) have missing evidence and are not classified NOT_APPLICABLE. Failed LPs: ['LP-12']. ... Detail: [{'provision_id': 'LP-12', 'tenant_text_len': 0, 'extraction_status': 'AMBIGUOUS', ...`
>
> **14 aborts across 18 full-pipeline attempts this arc.** It blocked verification work four separate times — two whole runs lost 4/4 attempts each in Step 457's predecessor, and one in Step 457 itself. This is independent of provider state and now dominates the cost of any measurement run. It is the single largest tax on this workstream.
>
> **3. RESOLVED (Step 461) — the two layering tests are retired and rewritten, suite full green at 352 passed, no test deleted.** They asserted that `lease_adapter`, `lease_extract` AND `lease_coverage` contain no reference to the 423 modules. **That was a not-yet-connected precondition, not a direction constraint** — correct while 423A/423C were standalone slices, and retired deliberately by the seam. The 423A test's own docstring gave its purpose as proving *"no Stage 5 verdict behavior changed as a side effect of adding it"*: a non-interference guarantee, with source-text `assertNotIn` as the proxy. A real layering rule constrains *direction*, that the 423 stack must not import coverage; these asserted the reverse, that the feature must never be used.
>
> Each is replaced by the check current doctrine actually requires — **the seam, and only the seam, reaches the 423 stack**: `lease_adapter` and `lease_extract` must not reference it at all, while in `lease_coverage` every reference must sit inside `_assemble_span_evidence`. A reference elsewhere in that module would mean a second, unreviewed entry point. Verified non-vacuous: adapter 0, extract 0, coverage module 1 / seam 1, and injecting a stray reference outside the seam makes the assertion fail. Setting `SPAN_EVIDENCE_LPS` empty or rolling the seam back leaves it passing — it constrains *where* the coupling may live, not whether it exists.
>
> **The genuine direction checks were left untouched and still pass**: `test_no_lp_taxonomy_leakage_into_span_resolution` and `test_evidence_spans_module_not_modified_by_this_slice`.
>
> **4. RESOLVED (Step 461) — `span_evidence_records` is live, and the stale comments are fixed.** The elicitor's own span-to-element attribution (`elicited_by`), plus offsets, `section_ref` and `verification_status`, is now persisted onto each seamed LP's assessment as `span_evidence_records`, attached in `_emit` — the single choke point every assessment passes through. **The span→element mapping is now a recorded measurement rather than a reconstruction.** Step 460 had to rebuild it from what each evaluator happened to quote, and says so. Provenance only; no verdict reads it, and no behaviour changed. The `EXPERIMENT, LP-07 ONLY` and `one edit is the whole rollback` comments now state the real scope and the real rollback.
>
...
> - **Direction 3 is the answer — but it is NOT "extend the architecture upstream". The architecture already exists, is already document-scoped, and has never been wired.** Census on 2026-08-20 (`build_log/FINDING_evidence_architecture_unwired.md`): `lease_adapter.py`, `lease_coverage.py`, `lease_coverage_305.py`, `lease_use_aware_coverage.py`, `lease_extract.py`, `app/main.py` and `app/job_manager.py` contain **zero references** to `lease_evidence_spans`, `lease_segmentation`, `lease_element_elicitation` or `lease_parameter_block`. Every caller of every public function in that stack is another module of the same stack, or a test. `propose_spans`, `resolve_proposed_spans`, `build_span_universe_sidecar`, `elicit_and_resolve_for_lp`, `build_elicitation_sidecar`, `dedupe_elicited_spans`, `attach_parameters_to_lp_evidence`, `check_gate_b` and `enforce_gate_b` have **no production caller at all**.
>   **423C needs no restructuring.** It already takes `canonical_source.canonical_text` (the whole document, never extraction output), is LP-blind at the search level (neutral "Target N" ordinals, never an `element_id` or LP id), and already dedupes on `(start_char, end_char)` with `elicited_by` as a **union** — multi-source spans are the existing design.
>   **The work is the seam.** `lease_coverage.py:150` is the entire decision about what each LP's evaluation sees: `tenant_text = (prov.get("tenant_text", "") or "") if prov else ""`, keyed off extraction's `provision_map`, plus the `all_lp_texts` map at `:83`. Replacing that one assignment with span-based assembly is the small part; the cost is in the consumers of its shape — the 305 prompt builder, negative-space detection, and the completeness gate, whose `fail_missing` test is `empty tenant_text` and has no obvious analogue once a span set is the unit.
>   **Same defect class as Step 452's `cmd_produce`: written, tested, never wired.** Both passed every check because no check asked whether the component was connected. Step 452 had four gate records, a 72/72 per-field producer-consumer census and a signed tag, and none asked whether `cmd_produce` had a body; the 423 stack has layering tests asserting its import direction, and a layering test cannot prove anything depends on the stack.
>
> **FIXED THIS WEEK.**
...
>
> **THE RUN.** Executed from a fresh clean detached worktree at P4, gate passing before the first call (`11 artifacts; HEAD=d679eec8525f; token=ef1a7af7f77d; commit_bound_via_signed_tag=stage2-sanction-431-ef1a7af7`). **108 role-calls**, 36 raw attempts × 3 roles, ~21 minutes. `cam/` clean before and after, commit unchanged across the run. No fatal; no terminal record written. All eleven package artifacts byte-unchanged by the run.
>
> **Canonical panel identity was enforced through provenance, exclusion, and retry — detection-and-exclusion, not prevention.** One `gpt-5.5` failure triggered an own-chain `gpt-5.4` substitution; that attempt was marked noncanonical, excluded from canonical N, and replaced by an additional attempt. It was the run's only degradation, on cand_04, the forcing candidate. Its `fallback_reason` `reasoning_exhaustion` is the classifier's *name* for "gpt-5.x returned empty content" (`_classify_failure`, `lease_coverage_305.py`) — **it is not an observation that a reasoning budget was exhausted**; no usage was recorded for the failed attempt.
>
> **TERMINAL CERTIFICATION-STATE DISTRIBUTION** (this is NOT §9.1 — an earlier status mislabelled it): 30 series — satisfied 14, `review_needed_disagreement` 11, `review_needed_no_qualifying_candidate` 5; `completeness: not_established` on all 30. Atreca, the known-good foil, returned `tenant_share` 4/5 and `rent_adjustment_pct` 0/5 with five disagreements: **replicate variance on the control, which constrains all reproducibility language.** Atlas's five `no_qualifying_candidate` outcomes **do not establish document-level absence of a rent adjustment.**
>
> **§9.2 cand_04.** No panelist grounded `operating_expenses` as applying to the 22.4% value — **0 of 18 role-judgments**; the field was `unclear` (role A, all six) or `none`. No opex assertion means no value-to-basis citation exists to examine. Under the v3.3 relation-bearing representation, the live panel emitted no false operating-expense linkage and the system produced no false certification. **The run did not isolate whether that outcome was caused by the prompt/schema representation, evaluator behavior, or their interaction. The policy-layer rejection of a positively asserted but false value-to-basis linkage remains unexercised.** cand_04's five `review_needed_disagreement` outcomes arose from `unclear` versus `none`, not from competing substantive assertions about the basis.
>
> **⛔ §9.1 IS UNPRODUCIBLE UNDER THIS PACKAGE — the central negative result.** §12 lists `431_validation.json` and `431_repository_seam_check.json` as unconditional Stage-2 outputs while marking their producers "(+ optional `validate_431.py` and seam-checker)"; §1 omits the producers entirely; the eleven token-bound artifacts include neither. The producers were never built, so the products cannot exist, and §8.2 forbids authoring the table in their place (*"COPIED FROM … never authored"*). **Nothing from this run may be labelled §9.1.** Root cause recorded in `build_log/448_code_status.md`; now prevented by **CLAUDE.md Rule 8 (producer-consumer census)**.
>
> **WHAT 431/447 SUPPORTS — the claim boundary.**
> - Governed evidence SELECTION over seven provisioned candidates across two leases, under a cryptographically sanctioned, repository-reproducible package, with per-panelist grounding, per-field agreement, and full decision traces recorded.
> - **Anti-borrowing enforcement at the VALUE layer IS exercised and trace-visible.** For every certified series the trace shows which candidate supplied `value_ok`, `support_ok`, `text_role_ok`, `relevance_ok` and `applicability_match`; the certifying candidate is identifiable per series and no certification was made by a candidate carrying a non-unanimous field (0 of 14). **This is a separate finding from package materialization and survives independently of the §9.1 gap.**
>
> - Detection-and-exclusion of a degraded panel member, with the excluded attempt retained in the audit record.
>
> **⚠ PROVENANCE QUALIFIER ON THE FINDINGS IN THIS BLOCK (recorded 2026-07-26, Step 451 producer-consumer census).** The value-layer trace-visibility finding above — and the 0-of-18 cand_04 result, the panel provenance table, and the count reconciliation elsewhere in this block — were **computed by post-run analysis over the immutable sidecar**, not emitted by any producer in the sanctioned package. P4 wrote three things: the sidecar, the runtime seam capture, and a thin report of terminal states. The interpreting scripts were written after the outcomes were visible and are bound by no execution-identity construction. The signed tag binds the package that generated the panel judgments; it does not bind the analysis that interpreted them. These findings rest on the sidecar's immutability, which is real, and not on the cryptographic construction, which does not reach them. **Attach this qualifier whenever they are cited.** Remedy in progress: `build_log/452_production_package_instruction_v8.md` (v8.2, RATIFIED 2026-08-15 at SHA-256 `b9343bd9…`, Stage 1A and Stage 1B only; Stage 2 separately unauthorized). Pointer updated 2026-08-15 — it previously named `452_production_package_instruction_v2.md`, an unbound superseded draft, and was two ratification cycles stale.
>
> **WHAT 431/447 DOES NOT SUPPORT — do not claim these.**
> - **Emitted certified evidence packages with materialized support spans.** No identifier with the prefix `semantic_support_span` is constructed, populated or emitted anywhere in the sanctioned package — 0 occurrences in the harness, the output schema, the selector prompt, and all three run outputs. The panel was never asked for one. **No evidence package with materialized support spans was emitted by this run.**
> - **Recorded anti-borrowing at the SUPPORT-SPAN layer.** §9.1 #3 ("No property borrowed from a semantic-support span…") quantifies over an empty domain. Any "true" would be **vacuously true** — true because nothing of the quantified kind exists, not because a mechanism declined to borrow. §8.2 couples the prohibition to a materialization requirement in one criterion (*"materialized, not value-only; no borrowed property"*): an empty domain satisfies the second conjunct only by failing the first.
> - **Gate-B handoff.** Not demonstrated by this run and not claimed.
>
> **SALVAGE POSITION.** Seven of the nine §9.1 criteria are computable from the immutable sidecar with **zero further model calls** (`build_log/431_postrun_partial_validation.json`, an explicitly post-run artifact that is **not** the preregistered validator and **may not be labelled §9.1**). #6 is not computable **as a property P4 exhibited** — the required field was never written, and fourteen certifications met its "if any" antecedent, so it resolves negative on the historical record. **#3 quantifies over an empty domain** (Step 449: zero occurrences of the `semantic_support_span` prefix anywhere in the package, including the schema and prompt — the panel was never asked for one); it is vacuous, not definitionally blocked, and an earlier framing in this document calling it a definitional question is superseded. **A complete §9.1 requires a corrected package but NOT a re-run.** Step 450 established that the raw material is present in the sidecar — `field_support` 108/108 across all six semantic fields, context citations carrying ids and verbatim quotes — so support-span materialization and grounding enforcement are constructible deterministically with zero further model calls. `build_log/452_production_package_instruction_v8.md` (v8.2, RATIFIED 2026-08-15 at `b9343bd9…` for Stage 1A/1B) specifies that package. Pointer updated 2026-08-15 from the superseded v2 draft.
>
> **=== END 447–450 BLOCK. The sanction-identity block below remains accurate as the record of HOW the package was sanctioned. ===**

# CAM Current State — Updated 2026-07-24 (431 STAGE-2 SANCTIONED — the sanction record; superseded on execution status by the 2026-07-26 block above)

> **=== SANCTION RECORD 2026-07-24 (superseded on execution status by the block above; the identity facts here remain operative) ===**
>
> **SANCTIONED EXECUTABLE IDENTITY (the operative facts — everything earlier in the token chain is superseded):**
> - **Package commit P4:** `d679eec8525fa672724a012f7d1fac0d0d8e7620`
> - **Sanction token T4:** `ef1a7af7f77d0999648bc39fa6b367a68d31d09470be699ee25555137cc511ca`
> - **Signed tag:** `stage2-sanction-431-ef1a7af7` (tag object `dba455a5821b3c7b1348619c1d820d39c4e98f5a`), peels to P4; `git tag -v` → *Good "git" signature for zvido@yahoo.com with ED25519 key SHA256:bHbLJOYJwDmI1E/cPduVFnOkbVpbRzwJEyAShQZuAHs*
> - **Sanction message:** SHA-256 `685d9dfe60cd9de5f808803ddee5507e8d6e15f1109f78c0b79de1a4c7674ec6`, materialized from Q-prep4 `779cc887ce0875ea3d50f6d52485217974e4828c`; the signed tag body is BYTE-IDENTICAL to the audited message.
> - **Q-final legibility record:** `56fe918701a66aa2a8fe3e8318dda7394d926eae` → `build_log/431_sanction_record_final.md`
> - **Eleven token-bound artifacts:** the five reviewed semantic artifacts (config, profiles, schema, selector prompt, fixture preflight) + the harness + the three authorization artifacts (allowed_signers, sanction key .pub, sanction policy) + BOTH lease fixtures (atreca, atlas).
> - **Superseded / never signed:** messages `56bce9e9…` (444), `84e6aab4…` (443), `3de7329c…` (442). Earlier tokens `47cb312a` (preregistration) → `833fd43e` → `9c2cc8e1` → `48054981` → `ce284b55` → `8d14543a` → `541989ef` → `0b98c6fa` → `8389e965` → `bb1c40b1` → `f341a188` → **T4**. Tokens before the line-ending correction were derived from Windows working-tree bytes and are NOT independently reproducible from the repository's LF blobs; they remain evidence of local drift gating only.
>
> **WHAT THE MEASUREMENT IS.** A preregistered measurement of the governed evidence-SELECTION mechanism over seven provisioned candidates across two leases (Atreca known-good foil + Atlas forcing cases), judged by a frozen three-model panel (A=claude-sonnet-4-6, B=gpt-5.5 at provider-default temperature, C=grok-4.3). Instruction: `build_log/431_partB_measurement_instruction.md` v3.3 RATIFIED. Blind-review provenance: `build_log/431_blind_review_provenance.md`. Two-stage gate throughout: Stage 1 built the preregistration package with the call path structurally `NotImplementedError`; Stage 2 (the live calls) required a separate cryptographic sanction, now granted.
>
> **THE LOAD-BEARING CORRECTION OF THE ARC (v3.3): the relation-bearing basis field.** `charge_basis_components` (which recorded which bases were *present/co-mentioned*) was replaced schema-wide by `value_applies_to_charge_basis_components` (which bases the candidate's OWN value applies to, established by cited value-to-basis linkage). The old field permitted a co-occurrence false match — a tax/CAM percentage co-located with an operating-expense clause could be certified as the opex share, with the panel classifying every fact honestly and the policy still certifying the wrong value. Found by a BLIND profile review, confirmed BLOCKING by the informed audit. `tenant_share.basis_match` = inclusion over the relation-bearing field; `building_share` unchanged (exact-match, unseeded — recorded amendment debt).
>
> **REVIEW CHAIN (all closed).** (1) BLIND Review A (prompt/schema, run incognito) — ZERO answer-key leaks; findings all response-shape. (2) BLIND Review B (profiles, stripped of every trace of the defect being tested) — independently CONFIRMED the co-occurrence fix closed; flagged `basis_match` as a basis-screen-not-share-screen (inert on these fixtures, recorded generality/claim-scope debt). (3) GPT INFORMED audit — sanctioned the measurement design. (4) Call-path scoped audit + a long sequence of provenance hardening (below). (5) Final construction audit — **CLEAR** on construction and the exact sanction message.
>
> **PROVENANCE HARDENING (Steps 432–446, all at zero call cost — each closed a real defect found by demanding raw bytes rather than narrative):** the reviewed package was a preregistration spec with no call path (implemented, then separately audited); a swallowed `FatalProviderError` that would have degraded the panel on a config-integrity violation instead of halting (fixed: type-halt on the propagating paths A/C, message-halt on the OpenAI wrapping path B); a long xAI excavation that ended in the code (xAI DOES invoke `_check_generation_integrity` at `provider_router.py:764`, records `last_integrity`, propagates fatals, and transmits temperature=0 unconditionally — the omission branch is structurally inapplicable to grok; several intermediate summaries asserting otherwise were false and are retracted); CRLF/LF non-determinism making the token unreproducible from committed blobs (fixed: path-scoped `.gitattributes` + committed-blob-derived token); a self-referential commit binding (fixed: Construction A — manifest carries NO commit SHA; binding lives in an external signed tag created after the package commit); a machine-local trust anchor (fixed: allowed-signers + key + policy committed AT the package commit, materialized from `HEAD:<path>`, with explicit key/principal/fingerprint enforcement); a manifest-trust bypass (the runtime took its *verification scope* from an editable working-tree manifest — fixed: HEAD-authoritative manifest, hardcoded artifact set, runtime token recomputation, four-way token equality); an imported-code bypass (Python loads from the working tree, so a modified `cam/core/provider_router.py` could execute under a valid tag — fixed: whole-tree cleanliness enforced by the gate + execution from a dedicated clean detached worktree, with `CAM_ROOT` derived rather than hardcoded); and an untracked measurement input (the Atlas lease was gitignored and absent from a fresh worktree — fixed: both leases tracked, LF-pinned, token-bound; `source_document_hash` verified INVARIANT under CRLF→LF and still equal to the frozen 430 values).
>
> **FOURTEEN ADVERSE TESTS all halt before any provider call**, each demonstrated in a clean detached worktree: modified/removed/added/omitted artifacts, altered hashes, stale tokens, substituted or multiple signing keys, stripped namespaces, wrong principal, missing anchor, modified imported module with tag verification stubbed to SUCCEED, untracked shadow module, edited lease fixture (22.4%→99.9%) with tag stubbed VALID, deleted lease. **And the positive case is demonstrated too:** at P4 with the signed tag present, the gate PASSES — four token sources agree, eleven artifacts verified, tree clean, bound via the signed tag.
>
> **CLAIM BOUND (must survive to the report and the patent record).** 431 measures GOVERNED SELECTION over these seven provisioned candidates — NOT general semantic accuracy, NOT recall, NOT production readiness, NOT share-vs-fee discrimination (recorded generality debt: `basis_match` is a basis screen; share-vs-fee identity is carried by the relevance/support/role conjunction). A `satisfied` cand_04 (the concept-substitution forcing case) is a GROUNDED PANEL ASSERTION that the 22.4% value applies to operating expenses — NOT proof it is legally correct, NOT validation that Atlas contains a genuine opex share. Grounded ≠ true: the mechanism governs whether a claim is cited and grounded, never whether it is correct.
>
> **NEXT: read the run.** ⛔ **SUPERSEDED 2026-07-26 — see the block at the top of this file.** This paragraph anticipated five execution artifacts. **Only three were produced** (`431_selection_measurement_sidecar.json`, `431_runtime_seam_capture.json`, `431_selection_measurement.md`); `431_validation.json` and `431_repository_seam_check.json` have no producer in the sanctioned package and cannot exist, so §9.1 is unproducible for this run. A high `review_needed_disagreement` rate remains an observed model-agreement result, not a mechanism failure — that part stands.
>
> **=== END 431 CURRENT-FRONTIER BLOCK — historical status blocks below (428 and earlier) remain the record. Note: the 429 "robust target resolution" work described below is a DIFFERENT (parameter-extraction) track from 431's evidence-SELECTION measurement. ===**
...
> **=== LATEST STATUS 2026-07-14 (read THIS FIRST — Step 428: Gate C parameter-assignment stability measurement. Verdict NO, 0/10 — but the values WERE found and silently discarded, not missing. No code changed.) ===**
>
> **428 — Gate C: parameter assignment stability (`build_log/428_gate_c_parameter_assignment_stability.md`; commit `db26530`, not pushed).** Measurement only, no code changed — `lease_parameter_block.py`, prompt, resolver, and normalization profile byte-identical to 427 (confirmed by `git status` showing no diff). **Plain verdict: NO — not stable enough to wire in. 0/10 extraction on every one of the four declared parameters (`tenant_share`, `building_share`, `rent_adjustment_pct`, `base_rent`), 0/10 Gate B passes, 10/10 aborts.** `extract_parameters()` returned an empty `parameters` dict on all 10 runs — not a partial result, not a shifted offset; nothing to compare across runs because nothing was produced in the first place. **Config-integrity: PASS** — identical prompt hash (`bbb1e99d0963887d`) and config hash (`7c2ac3de05b6e9ba`) across all 10 runs, matching 424/426/427; the 0/10 is not config drift. **Root cause — NOT a recall failure:** a supplementary diagnostic probe (run outside the official 10) called the same `elicit_spans_for_targets` with the same `PARAMETER_TARGETS`-derived element list and showed the model located and quoted all four values correctly — `100%`, `45.79%`, `3%`, and `$3.75 per rentable square foot` are all present, exactly as in 427's successful demonstration run. The failure is the `"target"` field: the schema asks for the bare ordinal `"Target 1"`, but the model echoed the full descriptive label (`"Target 1: Tenant's Share of Operating Expenses percentage"`). `extract_parameters()`'s `target_to_param.get(...)` mapping contains only the bare keys, so the lookup misses, falls back to the raw fuller string, that string is not in `PARAMETER_NAMES`, and the `continue` **silently discards the entire record — quotes included** — before `resolve_span` is ever called. This is why `parameters` came back empty on every run despite the model doing its job. Same defect class 421C documented (correctly-located evidence failing to reach its dependent LP via an unreviewed assumption between model output and gate), now one layer further down the stack; also the same `malformed_target_label` pattern 424 categorized (29 raw records) and 426 saw on a verified span — cosmetic there, total data loss here because `extract_parameters()` filters on `PARAMETER_NAMES` membership where `resolve_elicited_spans()` merely mislabels-and-keeps. **Gate B behaved correctly** — it detected all four unsatisfied dependencies and raised `GateAbortError` loudly on all 10 runs, zero false passes; Gate B is not the problem, it is faithfully reporting that the layer beneath it produced nothing to gate. This was caught precisely because the gate is fail-closed. **Next step is 429:** robust target resolution (parse the leading `Target N` ordinal), raise-on-unresolvable rather than silent discard/mislabel, a fixture built from real observed model output, then Gate C re-run before any wiring.
>
> **OWED DECISION (deferred, on the horizon) — LP-path terminal failure behavior.** 429 makes BOTH the parameter path and the LP path (`resolve_elicited_spans`) raise-on-unresolvable uniformly. On the parameter path that is terminal and correct (every parameter is Gate-B-load-bearing, so a call-level abort is right). On the LP path, uniform raise is strictly safer than today's silent mislabel-and-keep and is correct for now, but whether the *final* LP-path behavior should be call-level abort or a record-level unresolvable-target error routed to Review Needed — per "a failed trace kills the trace, not the evidence" (423 spec §7) — is an OPEN, tracked decision, deferred to a follow-on (429b or the wiring step). Gate C does not exercise the LP path, so this stays unmeasured by the 429 re-run. Do not resolve it inside 429.
>
> **=== LATEST STATUS 2026-07-14 (prior — Step 427: named parameter block + declared dependency map. LP-07's evidence context now contains the 100% tenant share for the first time in this project's record — built and tested, NOT wired into the live pipeline.) ===**
>
> **427 — Parameter block + dependency map (`build_log/427_parameter_block_dependency_map.md`).** New module `lease_parameter_block.py`. Four document-level parameters (`tenant_share`, `building_share`, `rent_adjustment_pct`, `base_rent` — never a provision, never an LP), extracted via the same element-guided elicitation call as LP elements but with a document-level, LP-unassigned target list. Declared dependency map, exactly `LP-02: [base_rent, rent_adjustment_pct]` and `LP-07: [tenant_share, building_share]` — no speculative entries, both justified by 426's 5/5 byte-stable measurement. Attachment is a pure dict lookup (`attach_parameters_to_lp_evidence`) — zero model discretion, proven non-destructive (the same `Parameter` object attaches to multiple LPs, identity-checked, unmutated). Gate B (`check_gate_b`/`enforce_gate_b`) fails closed via `GateAbortError` (imported, same doctrine as 422C) when a declared dependency lacks a verified span; `canonical` read explicitly, never inferred (422D doctrine); keyed to dependency names only — proven by tests grepping the gate functions' actual referenced names for absence of any lease-specific literal (`45.79`, `100%`, `3.75`, `3%`) and absence of any evaluator/vote reference. Demonstration run against the real Atreca document under `canonical_v2`: `LP-07` evidence context contains `tenant_share` (`"Tenant's Share of Operating Expenses of Building: 100%"`, offsets `[1942, 1996)`, verified) and `building_share` (`"Building's Share of Project: 45.79%"`, offsets `[1997, 2032)`, verified); Gate B passes cleanly. 22 tests, 334 total, 0 regressions. **Not wired into `lease_adapter.py`/`lease_coverage.py`** — Gate C (assignment stability) hasn't been built or measured yet, and 423 spec §8 blocks Stage 5 work until Gates A–D pass together. No selector panel; span-to-LP relevance beyond these four declared dependencies remains ungoverned.
>
> **=== LATEST STATUS 2026-07-14 (prior — Step 423C: element-guided, non-exclusive span elicitation. Slice three of the 423 spec; NOT wired into the live pipeline. Supersedes 423B's LP-blind approach.) ===**
>
> **423C — Element-guided elicitation (`build_log/423C_element_guided_elicitation.md`).** New module `lease_element_elicitation.py`, replacing 423B's LP-blind approach (423B's module is left in place, untouched, as a tested historical artifact — not deleted). Correction: LP-blindness was a means (stop partitioning) not an end (stop the model knowing what to look for) — 423B's blind call had no target and produced an 802-char multi-parameter blob plus zero proposals for the declared Controllable Expenses Cap element. 423C asks about one `expected_elements_305` element at a time (batched per-LP for cost — 32 calls, all model-facing labels neutral/ordinal, never an LP id), explicitly requiring repetition across elements and narrowest-passage (parameter-grained) quoting. The artifact stays LP-unassigned: a span's identity is its resolved offsets; `elicited_by` is audit-only provenance, never a routing/dedup key (proven by code-inspection test). Dedup rule: two elicitations resolving to the identical `(start_char, end_char)` collapse to one span with merged `elicited_by`; overlapping-but-distinct ranges never merge. Segmentation-call integrity mirrors 423B/421B: `canonical` explicit, never inferred from `fallback_used`. 28 tests, all model calls mocked; one bounded n=1 smoke run (LP-07 + LP-02, 2 of 32 LPs — not a full sweep) against the real Atreca lease produced `build_log/423C_span_universe_smoke_sidecar.json`: 11 raw quotes → 9 deduped spans → 8 verified/0 ambiguous/1 unverified. Diagnostic observations (not validation): Tenant's Share 100% and Building's Share 45.79% resolved as two separate, narrow, parameter-grained spans (contrast with 423B's single blob); Rent Adjustment 3% resolved as its own span; one clause was independently elicited by three different LP-02 elements and correctly deduped to one span with three provenance entries; the Controllable Expenses Cap produced zero quotes because **the real Atreca source file, as currently in the repo, contains no such clause** (confirmed by direct text search) — correctly not hallucinated, not a recall failure. 290 total tests (262 pre-423C + 28 new), 0 regressions. `lease_evidence_spans.py` (423A) not modified.
>
> **=== LATEST STATUS 2026-07-14 (prior — Steps 423A/423B: verified evidence-span substrate + LP-blind segmentation sidecar. Slices one and two of the 423 spec; NOT wired into the live pipeline.) ===**
>
> **423A — Verified evidence-span substrate (`build_log/423A_verified_evidence_span_substrate.md`).** New standalone module `cam/adapters/lease_review/lease_evidence_spans.py`: `CanonicalSource` (hashed, offset-addressable wrapper around `lease_parser.parse_document`'s output — one address space, no `page_ref`/`table_ref`) and `EvidenceSpan` (offset-addressed, code-verified quote; `verified`/`ambiguous`/`unverified`). Resolver never accepts model-emitted offsets — only verbatim quotes, resolved against the hashed source by code. Hard invariant: `normalize(canonical_text[start:end]) == normalize(span_text)`, re-checked before a span is returned `verified`. A span whose `source_document_hash` doesn't match the current parse is invalid and never silently re-resolved. 19 tests, including a seam test asserting `lease_adapter.py`/`lease_extract.py`/`lease_coverage.py` do not reference the new module — zero diffs to any live pipeline file.
>
> **423B — LP-blind span proposal sidecar (`build_log/423B_lp_blind_segmentation_proposal.md`).** New module `lease_segmentation.py`: an LP-blind segmentation/proposal model call (new prompt `segmentation_span_proposal.txt`, new schema `segmentation_schema.json` with `additionalProperties: false` enforcing no LP ids/verdicts/risk labels can appear in output) that proposes verbatim quotes with a structural-only span type (`clause`/`table`/`definition`/`other` — no semantic labels like "cap" or "carveout"), resolves them through the 423A substrate, and assembles a diagnostic sidecar artifact. Segmentation-call integrity mirrors 421B/416: `canonical` is an explicit metadata field, never inferred from `fallback_used` (the exact bug class 422D fixed, avoided here from the start). Canonical fail-closed: primary segmenter failure raises `SegmentationIntegrityError`; no fallback provider is implemented in this slice. 33 tests, all model calls mocked; one authorized n=1 plumbing smoke test against the real Atreca lease produced `build_log/423B_span_universe_smoke_sidecar.json` (18 spans, 18 verified, 0 ambiguous/unverified) — explicitly disclaimed as NOT a recall measurement or validation. Diagnostic note (not a finding): the 45.79% key-terms table did not appear among the 18 spans in this one run. **Evidence segmentation and LP selection are separated in architecture** (423 spec §2 doctrine); the live evaluator pipeline is unchanged — confirmed by seam tests asserting no live pipeline file imports `lease_segmentation` or reads the sidecar. 423B does **not** make LP-07 see the 100% tenant share; that requires the later global-parameter/dependency-map and many-to-many assignment slices (423 spec §5–§6), not yet authorized. Test count: 210 (pre-423) → +19 (423A) → +33 (423B) → **262 total, 0 regressions.**
>
> **=== LATEST STATUS 2026-07-14 (prior — Steps 422A/B/C: NOT_APPLICABLE schema hygiene, wiring, and completeness gate.) ===**
>
> **422A/B/C — NOT_APPLICABLE hygiene and extraction completeness gate.** Three steps, 2026-07-14. (A) Added `NOT_APPLICABLE` as a distinct extraction status for provisions structurally absent from a document type (LP-20/21/23/31 for industrial/warehouse leases). `KNOWN_ABSENT_BY_DOC_TYPE` registry in `lease_extract.py`; document-type-scoped, never global. (B) Wired the state end-to-end: returned-empty AMBIGUOUS provisions reclassified through the registry; `assess_coverage()` Step 2a bridge reads extraction status; `NOT_APPLICABLE` short-circuits to `coverage_state="not_applicable"` with provenance from `alignment_notes`. `check_extraction_completeness()` implemented as Gate 3. (C) Gate 3 wired into live Mode C pipeline (`run_lease_coverage_only()`) immediately adjacent to the 421B `extraction_failed` guard — same `GateAbortError`, same abort path. Canonical mode: any required LP with empty `tenant_text` not classified `NOT_APPLICABLE` aborts the run before Stage 5 coverage is reached. Non-canonical/debug: marks `run_degraded=True`, `extraction_completeness_failed=True`, `invalid_for_legal_analysis=True`. 31 new tests (8 new in 422C), 200 total, 0 regressions. Architecture spec for evidence assignment: `build_log/423_evidence_assignment_architecture_spec.md` — spec only, not yet authorized for implementation.
>
> **=== LATEST STATUS 2026-07-13 (prior — Step 421C: evidence-assignment architecture incident.) ===**
>
> **421C — Evidence-assignment architecture incident (`build_log/421C_evidence_assignment_incident.md`).** Read-only investigation. Root cause: Gemini's extraction is **destructive and exclusive** — each lease clause is assigned to exactly one LP bucket, and material content not assigned to a given LP is permanently outside that LP's evaluators' scope. Key findings: (1) LP-07 Operating Expenses: Exclusions list and Controllable Expenses Cap absent from 2/10 Gemini runs (minor hash `d3e62ead`); these are the provisions bounding tenant's unbounded exposure. (2) LP-12 Delivery: two structurally different clauses routed to LP-12 across runs — Condition Precedent (minor) vs Landlord's Work access rights (modal). (3) Key-terms table (Tenant's Share 100%, Building's Share 45.79%, Rent Adjustment 3%) at char 1,994 in source: never appeared in LP-07 under any Gemini extraction; appears in LP-00 (identity_check, not coverage-scored) in 2/10 Atreca runs; 0 hits across 101 Gemini-primary pipeline files and all 3 Atlas pipeline_results.json files. (4) **Grok exonerated:** Grok's LP-07 "missing" verdicts (attributed to evaluator instability in 417–420) were correct — the quantitative parameters were never in its evidence context. (5) **The numeric baselines from 417, 419, and 420 are VOID as system-performance baselines.** They measured a panel evaluating a lease whose key commercial terms were structurally excluded from the evaluators' evidence context. No figure from those steps — the overall stability percentages, the per-LP stability classifications, the Grok LP-07 flip attribution (43–49%) — may be cited as a measurement of system performance. They retain value as diagnostic artifacts documenting how the defect was discovered; that is their only valid use. All figures must be remeasured after evidence assignment is corrected. (6) **Atlas attorney validation scope:** validation findings are genuine reactions to what the lawyers saw; Operating Expense coverage specifically was assessed without the 100%/45.79% parameters in scope. (7) **Gemini non-determinism reversal (CRITICAL — voids a prior conclusion).** The initial 421 investigation (421A, preceding 421B) found six byte-identical Gemini extraction outputs in the 417/419/420 runs and concluded Gemini was deterministic at the observed output length. That conclusion was an artifact of the 32,000-token ceiling — the output was truncated to the same place every run and silently repaired by `_repair_truncated_json()` into something that appeared complete. With the ceiling raised to 65,000 tokens (421B), Gemini produces two distinct extraction hashes across 15 runs: 9× `f7f64b5c` (modal) and 6× `d3e62ead` (minor). **Gemini is not deterministic.** Any document stating or implying otherwise carries a voided conclusion. **Immediate architecture implication:** evidence-assignment must be non-exclusive (cross-cutting clauses assigned to all relevant LPs) and/or a global key-terms context must be injected into every LP's evidence. **What survives:** 414 (identity), 415/416 (config integrity), 421B (extraction integrity guard + ceiling fix) all valid. **Architecture spec:** `build_log/423_evidence_assignment_architecture_spec.md` (spec only, not yet authorized for implementation). **Schema hygiene and gate:** 422A/B/C (see block above).
>
> **=== LATEST STATUS 2026-07-13 (prior — Step 421B: extraction integrity guard implemented.) ===**
>
> **421B — Extraction integrity guard (`build_log/421B_extraction_integrity.md`).** Canonical extraction fail-closed guard implemented in `extract_provisions_single_doc()` — mirrors the evaluator guard from Step 414. Key changes: (1) `ExtractionIntegrityError` raised when primary Gemini fails in canonical mode; propagated as `GateAbortError` in `run_lease_coverage_only()` — run aborts rather than silently substituting a fallback extractor. (2) Stub-provision path blocked — if all extractors fail, the pipeline aborts rather than producing a legal analysis from empty evidence. (3) `EXTRACTION_MAX_TOKENS_SINGLE` raised from 32,000 to 65,000 to provide headroom above the observed 27–31k Gemini output range. (4) Raw failure capture: JSON parse failures now persist `raw_response_preview` (first 2,000 chars) in `meta.errors`. (5) Evidence hashes added to `pipeline_results.json`: `source_document_hash`, `extraction_output_hash`, per-LP `tenant_text_hash`, `extraction_attempt_chain`, `extraction_provider`, `extraction_model`, `extraction_fallback_used`. (6) 10 new tests in `test_421b_extraction_integrity.py`; 84/84 existing 414/416 tests still pass. Deferred: Gemini structured output (`response_mime_type`, `response_schema`) requires cam/core/ change out of scope. Retroactive audit (159 pipeline runs): 88.1% Gemini-primary, 11.3% fallback (all silent, now blocked), 5.0% stub-provision. Not pushed.
>
> **=== LATEST STATUS 2026-07-13 (prior — Steps 417/419/420: Stage 5 baseline N=10 runs; VOID as system-performance baselines per 421C.) ===**
>
> **⚠ BASELINE VOID NOTICE — these blocks exist to document how the evidence-assignment defect was discovered. Do not cite their figures as system-performance measurements.**
>
> **Steps 417, 419, 420 — Post-416 Stage 5 stability runs (N=10, Atreca fixture).** These steps ran the measurement specified in `build_log/417_post_416_stage5_baseline_spec.md`: N=10 keyed runs on the Atreca fixture, per-LP frequency distributions, per-role flip counts (A/B/C separated), variance source classification. The runs produced per-LP stability classifications, overall stability figures (including ~81.2% and ~43.8% figures), and a Grok LP-07 flip attribution of 43–49%. The initial 421 investigation (421A, preceding 421B) examined the extraction outputs from these runs and found six byte-identical Gemini extractions, concluding Gemini was deterministic at the observed output length.
>
> **Why these figures are void (established by 421C).** The evaluators in these runs assessed a lease whose key commercial terms were structurally excluded from their evidence context in every run. The key-terms table (Tenant's Share 100%, Building's Share 45.79%, Rent Adjustment 3%) was absent from LP-07's extracted `tenant_text` in every Gemini-primary run across all 101 known Gemini-primary pipeline files. The exclusions list and Controllable Expenses Cap — the provisions bounding tenant's unbounded Operating Expense exposure — were absent from the Gemini extraction in 2/10 runs (the `d3e62ead` minor hash). The Grok LP-07 flip attribution was not evaluator instability: Grok's "missing" verdicts were correct; the parameters were never in its evidence context. The Gemini determinism finding was an artifact of the 32k token ceiling (see item (7) in the 421C block above). **No stability figure, LP classification, or Grok attribution from Steps 417, 419, or 420 may be cited as a measurement of system performance.** These steps remain valuable as the diagnostic trail that led to the 421C incident characterization.
>
> **=== LATEST STATUS 2026-07-12 (read THIS FIRST — Steps 416b + 417 spec: config boundary documented; Stage 5 baseline scoped.) ===**
>
> **416b — Evaluator config boundary reporting (`build_log/416b_followup_reporting_gaps.md`; commit `1279cc3`).** Three reporting gaps from 416 closed.
>
> **Contamination boundary:** Commit `784efa7`, 2026-03-18. Two distinct stories: (1) **Role B primary (gpt-5.5)** was and remains at temperature=1 due to model capability — the model rejects non-default temperature at the API level; 416 preserves this exception explicitly; this is a standing constraint while gpt-5.5 is primary, not a closing window. (2) **Role B own-chain fallback / Stage 7 B (gpt-5.4)** was silently at temperature=1 from 2026-03-18 to 2026-07-12 due to the adapter bug; now fixed; contamination window closes at 416 commit `a0fe4a3`.
>
> **Parameter enumeration:** Full sweep across all four adapters. No additional silent drops found after 416. top_p / top_k / penalties / seed / JSON mode are not declared on ModelTarget — no assertion is needed or missing for them. GoogleGenAIAdapter does not call `_check_generation_integrity` but transmits temperature correctly; Google is fallback-only, not a primary evaluator.
>
> **Assertion scope:** `_check_generation_integrity()` is partial by design. Hard-fails on temperature (undocumented omission) and max_tokens (absent). Soft-records reasoning_effort absence (model-gated; not a silent drop). Does not assert undeclared parameters. Complete over the three declared evaluator-critical parameters; not total over the full provider parameter space. Future ModelTarget fields must be added to `EVALUATOR_CRITICAL_PARAMS` to be caught.
>
> **417 spec — Post-416 Stage 5 baseline (`build_log/417_post_416_stage5_baseline_spec.md`; NOT YET RUN).** Scoped the measurement that establishes residual Stage 5 wobble rate after 414/416. Spec requires: Atreca fixture (same as 411), N≥10 (floor not preference), per-LP frequency distributions (not binary stable/churn), per-role flip counts (A/B/C measured separately), variance source classification (fallback/config/temperature/prompt/aggregation separated). The measurement answers: how much of the pre-416 ~31% wobble remains; is Role B primary a disproportionate source; are Claude/Grok still producing temperature=0 variance; what stabilization framing dominates. **The measurement run is not yet authorized.**
>
> **=== LATEST STATUS 2026-07-12 (read THIS FIRST — Steps 415–416: temperature config trace + evaluator config integrity fix.) ===**
>
> **415 — Temperature / config transmission trace (`build_log/415_temperature_config_trace.md`).** Read-only measurement. **Primary finding: Role B (GPT-5.5 / gpt-5.4) never received `temperature=0` before this step.** The OpenAI adapter had a broad `startswith("gpt-5")` branch (introduced 2026-03-18, commit `784efa7`) that unconditionally dropped `temperature` from the API payload for all gpt-5.x models. Config declared `temperature=0.0` for Role B in all three stages (Stage 2 / 5 / 7); the adapter silently dropped it. Roles A (Claude) and C (Grok) transmitted temperature=0 correctly. No seed/determinism parameter is set anywhere in the lease pipeline; Anthropic and Google don't expose one; OpenAI/xAI support it in principle but undocumented for the current model generations.
>
> **416 — Evaluator config integrity fix (`build_log/416_temperature_config_integrity.md`; commit `TBD`).** Live probes (2026-07-12) established: gpt-5.5 rejects ALL non-default temperatures — `BadRequestError: "temperature does not support 0 with this model. Only the default (1) value is supported."` gpt-5.4 accepts `temperature=0`. gpt-5.2 accepts `temperature=0` (the original adapter comment was factually wrong about gpt-5.2). Fix: replaced broad `startswith("gpt-5")` exclusion with explicit `TEMPERATURE_ONLY_DEFAULT_MODELS = {"gpt-5.5"}` capability set, documented with probe evidence. Added `_check_generation_integrity()` assertion function: every evaluator call now compares declared config to outbound payload — undocumented parameter omission raises `FatalProviderError`. All three provider adapters (OpenAI, Anthropic, xAI) updated. 32 new tests in `test_416_config_integrity.py`; 52 existing 414 tests still green. **Impact: gpt-5.4 (Stage 7 Role B) and gpt-5.2 now transmit temperature=0. gpt-5.5 (Stage 2/5 Role B primary) cannot honor temperature=0 — this is a model capability gap, not an adapter defect; the exception is now documented and logged rather than silent.**
>
> **Role B primary (gpt-5.5) temperature status after 416:** gpt-5.5 is identity-frozen but not temperature-config-frozen. The declared `temperature=0.0` cannot be transmitted; the adapter logs the capability exception on every call. Role B fallback (gpt-5.4) now correctly transmits temperature=0. This is a model-provider capability gap — not fixable by adapter changes. The panel was always identity-frozen (414); it is now also config-integrity-asserted (416). These are separate invariants.
>
> **Benchmark caveat boundary:** The broad temperature-dropping branch landed 2026-03-18 (commit `784efa7`). Runs before that date had different adapter behavior (initial deployment 2026-03-16). All evaluator runs between 2026-03-18 and the 416 fix have Role B operating at provider-default temperature=1 for gpt-5.5, and dropped temperature for gpt-5.4/gpt-5.2 fallback. This caveat is scoped to that date boundary — not an unbounded caveat on all historical numbers.
>
> **=== LATEST STATUS 2026-07-08 (read THIS FIRST -- newest frontier. 408-410 compound-consequence + reproducibility arc CLOSED; Priority Exposure re-gated on Stage 5 stabilization. Next: Step 411 Stage 5 Coverage Reproducibility Trace.) ===**
>
> **One-line conclusion:** Compound-consequence lane (408C) is architecturally sound and proven stable on stable input, but CRX findings are nondeterministic across runs (0/7 IDs stable, 1/7 semantically stable) because Stage 5 coverage-state nondeterminism is the primary causal layer. Priority Exposure remains BLOCKED until Stage 5 is stabilized.
>
> **408C -- compound-consequence implementation (SHA `450f52d`).** Dedicated `_COMPOUND_FINDING_SYSTEM_PROMPT` lane (neutral, interaction-focused, not LP-consequence-reuse), `compound_`-prefixed fields on each CRX object, no LP-level contamination (LP-27 independence confirmed), forbidden-token scan PASS, 7/7 CRX assessed both A and B runs. Architecture proven sound. LP-22/LP-26 subordination-trap is the control case: consequence assessed identically on stable compound input. Stability lives at the consequence layer; instability is upstream.
>
> **409 -- compound reproducibility trace (SHA `155c0c1`).** 0/7 CRX finding_ids stable across the two 408C Atreca runs. 1/7 semantically stable (LP-22/LP-26 subordination trap, Jaccard=1.00). All other CRX findings diverged on LP-set or pattern-type. Consequence churn traced to upstream Stage 7 identity instability, NOT 408C. Priority Exposure blocked: cannot rank findings that are redrawn each run.
>
> **410 -- Stage 7 candidate instability trace (report: `build_log/410_stage7_candidate_instability_trace.md`; SHA `87a9fa6`, local, not pushed).** Read-only boundary measurement. Three nondeterminism layers documented: **Layer 1 (PRIMARY, MEASURED)** = Stage 5 coverage-state nondeterminism. Run A flagged 18 LPs; Run B flagged 19 (LP-01/LP-15 in A only; LP-09/LP-28/LP-29 in B only). Stage 7 Pass-1 prompt hashes differ (db31097... vs c7049eb...). `_build_compound_user_prompt` (lines 588-619 in `lease_synthesis.py`) embeds ALL 32 LPs' coverage states in the compound matrix, so Stage 5 variance propagates directly into compound prompt content -- not just the flagged-LP list. **Layer 2 (SECONDARY, inferred)** = cross-provider LLM nondeterminism at temperature=0; already in place; not a standalone fix. **Layer 3 (STRUCTURAL, confirmed)** = doubly-unstable CRX ordinal ID assignment: `_cluster_compound_candidates` L953 assigns first ordinal by dict insertion order; `_dedup_compound_findings` L1661 re-assigns by union-find root index -- two independent positional label sites, no content-based key anywhere in the pipeline. Temperature=0 is already set and is NOT a fix for Layer 1 or 3. Compound sidecar gap: Step 386 instruments the directional Pass-1 family only; the compound call has no equivalent sidecar. Fix sequence (not yet built): stabilize Stage 5 -> add content-keyed compound identity -> measure residual Layer 2 variance.
>
> **DECISION:** Priority Exposure remains BLOCKED. Stage 5 nondeterminism is the next measurement target before any compound-identity or compound-consequence architecture change.
>
> **NEXT: Step 411 Stage 5 Coverage Reproducibility Trace** -- read-only measurement on the same Atreca A/B artifacts. Which LPs change coverage state across runs? Which elements drive the verdict crossing? Is nondeterminism concentrated on specific LPs or spread broadly? Compare Stage 5 outputs element-by-element across runA and runB `pipeline_results.json`.
>
> **STANDING CAVEATS:** All findings DIRECTIONAL, two leases (Atlas + Atreca), NOT promoted, NOT patent record. Priority Exposure blocked pending Stage 5 stabilization. Widened LP-level Stage 5e gate stays DEFAULT-OFF.

> **LATEST STATUS 2026-07-03 (read before the 2026-06-12 block below) — Phase 1 external review COMPLETE (interim) + context-dependency VISIBILITY build handed off.**
>
> **Phase 1 external CRE review — complete (interim), n=3, PROVISIONAL, NOT promoted.** Three experienced CRE attorneys answered the blind packet: R1 Yaima S. (blind), R2 Max L. (blind, REIT in-house/ex-Dentons), R3 Josh (cold read, project-aware — near-blind tier, not a stranger). Canonical record: `build_log/lawyer_packet_COMBINED_RESULTS.md`. R3 detail + capstone: `build_log/R3 josh and corrections.md`. Interim synthesis: `build_log/Phase1_External_Review_Interim.md`.
> - **Clear calls: reviewers converge with tool + each other** (BOKF Risk, late-delivery Risk, materiality, §34-vs-§3.1(C) credit, min fix).
> - **Disagreement concentrates on the tool's already-unstable clauses** (LP-19, LP-26), NOT at random — tool wobble tracks genuine lawyer wobble.
> - **LP-19:** leans 2-1 toward genuine contestability; the two "depends" votes (R1, R3) CLUSTER ON THE SAME REASON (tenant-operational context). Cleanest near-resolution. R2 confident is the dissent. A clean blind R4 would settle it.
> - **LP-26:** three-way bucket split (Risk/Standard/Improvement); multidimensional (context / market-standard-drafting / drafting-norms). New falsifiable claim (Josh): §19.2 SNDA structure may be ATYPICAL ("backwards") — candidate explanation for the instability. "Unusual drafting + confident" is real → unusual ≠ ambiguous.
...
> - **Open frontier: NO DETECTOR** for which clauses are context-dependent — the real next measurement question.
> - **DISCIPLINE:** all of the above is n=3, provisional, panel-gated. NOT promoted to `Patent_Current_State.md`. Hypotheses stay in the holding pen (H-STD-01 / H-IMP-01 / H-BORDER-01, `build_log/H-STD-01_candidate.md`); NONE promoted. Named methodology failure mode recorded: **reason-without-provenance** (a reason detached from {reviewer, clause, question} can manufacture a false finding — caught live this session).
>
> **Context-dependency VISIBILITY build — Part 1 SHIPPED (local, unpushed; verified against artifact 2026-07-05).** Steps 398 + 398b implemented by Claude Code (commits `20f5095`, `758bdf6`, local/unpushed). `use_impact` was ALREADY at the Mode C render point in `app.js` (Step 375G renders `use_reasoning` as an indigo "Client Impact" box). 398 added a distinct amber "Context Dependency" block; 398b reduced it to a thin SIGNAL STRIP (consequence tag + purple "evaluators split 1-1-1" note only) so `use_reasoning` is NOT duplicated between the two boxes. Verified 2026-07-05 by reading the `buildItem()` render block directly against a real run: for a `context_dependent` + 1-1-1 LP-05 item, Client Impact (indigo, reasoning) renders above Context Dependency (amber, split note), no duplication — logic confirmed correct without needing the browser. Display-layer only; no `cam/core/`, prompt, model, or pipeline change. NOTE: the `context_dependent`-without-1-1-1 branch (amber tag, no split note) exists in code but has NO live data yet to exercise it. Status file: `build_log/398_code_status.md`. **Part 2 (Josh's deal-dependency, distinct from use-profile) DEFERRED — needs pipeline/prompt change.**
>
> **Run-to-run instability observed (DIRECTIONAL, n=2, NOT promoted):** same lease (`atlas_meridian_warehouse_lease.txt`), same clause (LP-05 Permitted Use), classification FLIPPED across runs — `context_dependent`/1-1-1 (2026-06-11) vs `beneficial`/2-1 (2026-07-05 run `1771a4`). Consistent with the known LP-05 wobble / DEF-010 consequence-nondeterminism and with the "no detector for context-dependency" frontier (the clause that flips is the under-determined one). Recorded as pattern; NOT a finding, NOT in the patent record. Consequence for verification: a FRESH Atlas run cannot be relied on to reproduce the strip-firing item; the frozen 2026-06-11 run is the guaranteed-hit artifact.
>
> **[SUPERSEDED — original handoff note kept for provenance]** Key finding from reading real source this session: the "missing bucket" the reviewers pointed at is **largely ALREADY COMPUTED** — Stage 5e (`cam/adapters/lease_review/lease_use_impact.py`) produces `use_impact` per coverage item (`use_consequence` incl. `context_dependent`, `use_reasoning`, `materiality`, `evaluator_agreement`), it IS persisted to `pipeline_results.json`, and IS served to the client intact via `get_job_results` in `app/main.py`. It is simply **rendered NOWHERE user-facing** (not in synopsis PDF, not in chat context; on-screen `app.js` render TBC). So the gap is VISIBILITY, not computation. Part 1 = surface it in the UI (display-layer only). Handoff: `build_log/398_chat_instruction.md`. Guardrails: no `cam/core/`, no new model call/prompt, no hardcoded content, don't call it a "bucket" ("Context Dependency"/"What this depends on"), strictness=display-only, render only when real data, STOP-and-report if `use_impact` not actually reaching `app.js`. **Part 2 (Josh's deal-dependency, distinct from use-profile) DEFERRED — needs pipeline/prompt change.**
>
> **Next moves (untimed unless noted):** (1) ~~Claude Code executes 398~~ DONE + verified (see SHIPPED note above); remaining closeout is UI polish of the strip. (2) Willingness-to-pay UI signal — narrowed to "does NAMING THE MISSING CONTEXT make the output more useful?" (structural read + named dependency + the question-to-answer vs bare bucket); run opportunistically 1-on-1, NOT via recruited panel. (3) Attorney prep (method/evidence/roadmap structure) — has the calendar: conversation before September, non-provisional ~November. Reviewer phase is NO LONGER GATING. Do NOT expand to new domains/modules (instance one not yet monetized). Do NOT build a detection-altering strictness setting.
...
> **=== LATEST STATUS 2026-07-05 (read THIS before the 2026-07-03 block above/below — it is the newest frontier). UI provenance session + KEY ARCHITECTURE FINDING. Next = Stage 5e sparsity. ===**
>
> **Full continuity + all commits: four dated UPDATE blocks at top of `Docs/NEW_THREAD_PROMPT.md`. Next-session spine: `build_log/NEXT_stage5e_sparsity_investigation.md`.**
>
> **Shipped this session (all local, UNPUSHED, main):** 399 strip reword (`9d8ede2`); 400 provenance chips (`282abb9`+`4f6e2c5`, exposure Lease-specific/Default + materiality Assessed/Default, display-only, fields already reached `buildItem`); 402 sidebar (`f52072e`, title 2-line wrap + section default-collapse); 403 (`24d4767`, RISK also default-collapsed, landing tab already Overview). localStorage caveat: prior toggles override new collapse defaults — test in fresh window.
>
> **THE KEY FINDING (399c classifier trace, `f663f9d`, trace-only):** `partial`-coverage routing in `classifyFindingType()`/`sevTriage()` is **BLIND to `use_consequence` direction** — severity is from `use_impact.materiality` + `partial_class` only; `harmful` is never read for partial cards. Twin proof: LP-16 Parking and LP-10 both partial/medium/harmful, but LP-10 (no hard_flag) → IMPROVEMENT while LP-16 → NEEDS REVIEW only because hard_flag (verdict_distance-driven) floored it up. **Conclusion (now MECHANICALLY justified): RISK is confidence/materiality-first; consequence is orthogonal to the buckets; Priority Exposure MUST be a surface ABOVE the buckets, NOT a RISK redefinition.** Do NOT redefine Risk. Do NOT patch `sevTriage()` `medium&&harmful→HIGH` (parked policy, not a hotfix).
>
> **Priority Exposure readiness (399 census + 399b two-run):** assessed `use_impact` reaches only **7–9 of 32** cards (7 on 2026-06-11 `3131a1`, 9 on 2026-07-05 `443e33`); 30/32 exposure statements schema-default. Sparsity REPRODUCED across two runs (no longer n=1; still one lease). Badge confirmed verdict_distance-driven both runs. Diagnosis: provenance + coverage, NOT ontology. Artifacts: `399_priority_exposure_readiness_census.md`, `399b_two_run_census_comparison.md`, `399c_classifier_boundary_trace.md`.
>
> **NEXT MAJOR INVESTIGATION = Stage 5e `use_impact` sparsity (pipeline, read-only first).** Why only 7–9/32 assessed, why the set moves run-to-run. CODE read of Stage 5e invocation/eligibility FIRST; instrument a run ONLY if code doesn't explain it. BLOCKER for Priority Exposure. Spine: `build_log/NEXT_stage5e_sparsity_investigation.md`. Stage 5e = `cam/adapters/lease_review/lease_use_impact.py` — confirm frozen-vs-infra before any change.
>
> **Also open (not blocking):** 401 badge split (`build_log/401_badge_split_SPEC.md`, display-only, lower priority); strip conditional-fire (LP-05 hit `context_dependent` only 1 of 3 runs). ALL 2026-07-05 findings DIRECTIONAL, one lease, NOT promoted, NOT patent record.
>
