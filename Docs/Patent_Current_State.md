...
> **ACTIVE DECISION REGIME — 2026-08-23 THROUGH 2026-11-11.** The project is now running the bounded process in `Docs/CAM_Patent_Go_No_Go_Plan_2026.md`. The November 11 decision is whether CAM's expected IP and commercial option value justifies funding and authorizing non-provisional preparation. It is not a commitment to file, a product-completion deadline, or a general CAM engineering roadmap.
>
> **FILED-BASELINE CONTROL — RECEIPT CONFIRMED 2026-08-23.** The USPTO Electronic Acknowledgement Receipt confirms provisional application **63/980,196**, received **2026-02-11 at 10:46:05 AM ET**, docket `DAUM-U001`, confirmation number `8977`. It identifies ten submitted documents, including `DAUM-U001 CLAIMS-CLM.docx` (11 pages), `DAUM-U001 SPECIFICATION-SPEC.docx` (47 pages), and `DAUM-U001 INITIAL DRAWINGS.pdf` (17 pages). The exact receipt manifest and local-copy verification status are recorded in `Docs/Filed_Provisional_Manifest_2026_02_11.md`. Counsel states that the non-provisional must be filed on or before **2027-02-11** and requests authorization preferably two to three months beforehand. The project's **2026-11-11** decision date is the internal authorization date, not the statutory deadline.
>
> **COUNSEL ATTACHMENT SET LOCATED — PARTIAL BYTE VERIFICATION.** The complete attachment set from counsel's filing-confirmation email is preserved at `Docs/Patent/privilegedconfidentialdaumu001provisionalpatentapp/`. Counsel expressly described these as copies of the filed documents. Seven of ten substantive attachments match the receipt's SHA-512 digests exactly. The ADS, claims, and specification do not. For support review, use the counsel-provided `DAUM-U001 CLAIMS.docx` and `DAUM-U001 SPECIFICATION.docx` in that folder rather than the older working copies, but state that their bytes do not match `DAUM-U001 CLAIMS-CLM.docx` and `DAUM-U001 SPECIFICATION-SPEC.docx` as submitted. Full results: `Docs/Filed_Provisional_Manifest_2026_02_11.md`.
>
> **CURRENT REQUIRED WORK PRODUCTS.** (1) counsel/Patent Center explanation or digest-matching copies for the ADS, claims, and specification; (2) provisional-support/new-matter map using the counsel-provided package with the qualification above; (3) claim-priority matrix weighted for novelty, Section 101, detectability and design-around resistance; (4) closest-art memo; (5) current external monetization evidence; and (6) priced filing options from counsel. Preserve all dated supplements and bounded/void findings as history; do not rewrite them to fit the November decision.

**Last updated:** 2026-08-26 (**bound-on-reading 2026-08-26 added: both deployed runs of Step 487 ran a SUBSTITUTED evaluator panel -- role A served by gemini-2.5-pro, undisclosed on every surface and misreported by the per-element provenance fields. Frozen L1 measurements are untouched; the bound governs how future DEPLOYED measurements are read.** Prior: 2026-08-23 (**bound-on-reading §3 added: the extraction-layer defect has a demonstrated fix in the coverage path; the §1 cand_04 bound is UNCHANGED and the frozen L1 measurement is untouched.** Prior: 2026-07-26 — **431/447 — the Stage-2 run EXECUTED: 108 live calls under the signed sanction. The panel-governed SELECTION claim is DEMONSTRATED IN PART. §9.1 mechanism-success is UNPRODUCIBLE under the sanctioned package — its validators were never built — so no §9.1 result exists or may be cited. Claim boundary below.** Prior: 2026-07-24 — sanctioned, handed off. 2026-07-21 — Stage-1 sanctioned, not yet run. 2026-07-14 — Supplement #26 written.))

> **=== BOUND ON READING, ADDED 2026-08-26 (Steps 487–488). NOT A RETRACTION. ===**
>
> **A measurement taken from a run whose evaluator panel was substituted is not a measurement of the frozen three-model panel.**
>
> The frozen panel is A=`claude-sonnet-4-6`, B=`gpt-5.5`, C=`grok-4.3`. **Both deployed runs of Step 487 had role A served by `gemini-2.5-pro`** — 202 of 202 element verdicts on one run, 196 of 202 on the other, with the remaining 6 being stubs that no model produced. Every Anthropic call in the entire deployed record failed (`api_error`, hard class, same-provider retry also failed, 60 of 60). There is **no observed window in which Anthropic served this pipeline**, and no onset time is recoverable: all 30 fallback events in a run share one assembly-time timestamp.
>
> **Agreement observed on a substituted panel is not panel agreement.** Where the deployed runs agree — including the three seam-sourced issue areas LP-07, LP-12 and LP-27, whose `elements_found` lists are byte-identical across both — that agreement is between two runs of *the same substituted configuration*, not between two runs of the specified one. It is evidence of run-to-run stability under substitution; it is not evidence about the frozen panel.
>
> **Nothing in the output discloses the substitution.** `run_degraded=True` and `degraded_reason='evaluator_fallback'` are set in the stored result, but every presentation surface — the results page, the annotated DOCX and PDF, the summary DOCX, the batch DOCX and the combined synopsis PDF — keys on extraction-completeness fields that are False on such a run, and `run_quality` never reaches the polled job status. Worse, the per-element provenance fields **misreport** it: on the six stub verdicts, `actual_model`, `actual_label`, `label` and `is_fallback: false` all name the model that produced nothing, and a census run over those fields returns the wrong answer silently. Full record: `build_log/FINDING_evaluator_substitution_is_unmarked.md` and `build_log/487_code_status.md`.
>
> **This does not alter any frozen L1 measurement.** Step 447 and the 431/447 selector-panel run executed as sanctioned against immutable artifacts, before this configuration existed; their outputs are unchanged and unchangeable by later work. Nothing above is a re-run, a re-analysis, or a correction of them.
>
> **What it changes is how any FUTURE deployed measurement must be read.** Before a deployed run is cited for anything — a rate, an agreement, a comparison against a local baseline — **the panel that actually served it must be established from `fallback_events` and the per-element `is_fallback` flags, and the stub caveat applied.** A deployed run is not self-certifying as to its own panel, and the fields that appear to certify it are not reliable. **Not established, and deliberately not checked here:** whether `all_failed` stubs of this kind appear in any earlier local run or in the frozen 431/447 artifacts.
...
>
> **=== END EXECUTED-RUN BOUNDARY. The sanction-identity note below remains accurate as to HOW the package was sanctioned. ===**

> **=== 431 SELECTOR-PANEL RTP — STATUS NOTE (2026-07-24; sanction identity. SUPERSEDED as to run status by the 2026-07-26 block above; identifiers remain operative). ===**
>
> **SANCTIONED IDENTITY (cite these; the 07-21 note's `65556ee`/`47cb312a` are superseded):** package commit **P4 `d679eec8525fa672724a012f7d1fac0d0d8e7620`**; sanction token **T4 `ef1a7af7f77d0999648bc39fa6b367a68d31d09470be699ee25555137cc511ca`**; signed tag **`stage2-sanction-431-ef1a7af7`** (object `dba455a5821b3c7b1348619c1d820d39c4e98f5a`), peeling to P4, verifying as *Good "git" signature for zvido@yahoo.com with ED25519 key SHA256:bHbLJOYJwDmI1E/cPduVFnOkbVpbRzwJEyAShQZuAHs*; sanction-message SHA-256 `685d9dfe60cd9de5f808803ddee5507e8d6e15f1109f78c0b79de1a4c7674ec6`, the signed bytes byte-identical to the independently audited message; legibility record `build_log/431_sanction_record_final.md` at `56fe918701a66aa2a8fe3e8318dda7394d926eae`.
>
> **STATUS DISCIPLINE (do not soften).** ⛔ **SUPERSEDED 2026-07-26 — the run has now executed; see the block above for what it does and does not support.** As written on 07-24 this said: The measurement is preregistered, frozen, adversarially reviewed, cryptographically sanctioned, and **handed off for execution**. Until the run returns and its report is written, RTP of the selection panel is **EXECUTING, NOT YET DEMONSTRATED** — no result, distribution, or certification outcome exists in this record, and nothing about panel selection *performance* may be cited. What CAN be cited today, dated and independent of the run's outcome: the two-stage build/run gate; the frozen semantic artifacts; the blind-then-informed review method; the relation-bearing-field correction; and the execution-identity construction below.
>
> **EXECUTION-IDENTITY CONSTRUCTION (design contribution, reduced to practice regardless of the run's outcome).** The measurement is bound to its authorization so a third party can verify it from the repository alone: (i) artifact identity derives from **committed Git-blob bytes** under path-scoped LF pinning, so the token is reproducible on any platform (an earlier CRLF-dependent token was not, and is recorded as such rather than retroactively repaired); (ii) the package manifest carries **no commit SHA** — a manifest inside commit C cannot name C — so the run-commit↔token binding lives in an **external signed annotated tag created after the package commit**, escaping the self-reference; (iii) the **trust anchor is committed at the package commit** (allowed-signers, sanction public key, authorization policy naming principal and fingerprint), materialized at runtime from `HEAD:<path>` rather than inherited from local config, with authorized key, principal, namespace, and tag-body fields enforced explicitly rather than by parsing verification prose; (iv) the runtime treats the **committed** manifest as authoritative, enforces a **hardcoded artifact set**, recomputes the token from HEAD blobs, and requires four-way token equality (recomputed ≡ committed manifest ≡ invocation ≡ signed tag); (v) execution occurs from a **clean detached worktree at the package commit** with whole-tree cleanliness enforced by the gate, binding all repository-local imports — not merely the declared artifacts — to the sanctioned commit; (vi) the **measurement inputs themselves** (both lease fixtures) are tracked, LF-pinned, and token-bound, so the package is self-contained. Fourteen adverse tests demonstrate a pre-call halt for each attack class (modified/removed/added artifacts, altered hashes, stale tokens, substituted or multiple signing keys, wrong principal, missing anchor, modified imported module and untracked shadow module with signature verification stubbed to succeed, edited or deleted lease fixtures); the positive case — gate PASSES at the sanctioned commit under the signed tag — is demonstrated as well. **The sanction act is the inventor's**: the tag is created personally, with a dedicated passphrase-protected key held outside the build environment, over a message whose exact bytes were audited before signing.
>
> **REVIEW-METHOD ADDENDUM.** Throughout the hardening sequence the reviewing party repeatedly refused to clear a construction from a narrative *about* the bytes, requiring the literal committed objects. That discipline surfaced defects no summary had revealed: CRLF non-determinism, a machine-local trust anchor, a manifest-scope bypass (the runtime took its verification scope from an editable working-tree file), an imported-code bypass (Python loads from the working tree, so a modified provider module could execute under a valid tag), and an untracked measurement input.
>
> **=== END 2026-07-24 NOTE. The 2026-07-21 note below remains accurate as to the v3.3 relation-bearing-field correction and the blind-then-informed review method; its package/token identifiers and "not yet executed" status are superseded. ===**

> **=== 431 SELECTOR-PANEL REDUCTION-TO-PRACTICE — STATUS NOTE (2026-07-21). Read this before citing Supplement #26's "DESIGNED-NOT-BUILT: selector panel" line — 431 is the RTP of that exact claim, and its status is AUTHORIZED-NOT-YET-DEMONSTRATED. ===**
>
> **What 431 reduces to practice.** Supplement #26's fourth contribution — *panel-governed evidence SELECTION with an asymmetric merge rule* — was DESIGNED-NOT-BUILT as of #26. Step 431 is its preregistered reduction to practice: a frozen three-model panel (A=claude-sonnet-4-6, B=gpt-5.5, C=grok-4.3) makes blind, citation-grounded classifications of seven provisioned candidate spans, and a deterministic policy certifies whether each declared parameter is satisfied — the panel judges candidates, the policy certifies parameters, completeness limits every negative claim (Part A doctrine). Instruction: `build_log/431_partB_measurement_instruction.md` (v3.3 RATIFIED). Frozen package commit `65556ee`, sanction token `47cb312a`.
>
> **STATUS DISCIPLINE (do not soften — the run has NOT happened).** The measurement is *preregistered, frozen, adversarially reviewed, and sanctioned for execution* — but **zero model calls have been made in the entire 431 arc.** RTP of the selection panel is therefore **AUTHORIZED, NOT YET DEMONSTRATED.** No result, no distribution, no certification outcome exists yet. Nothing about panel selection *performance* may be cited until the Stage-2 run executes and its report is written. What CAN be cited today as conception-plus-preregistration: the two-stage build/run gate; the frozen selector prompt/schema/profiles; the blind-then-informed review method (see below); and the relation-bearing-field correction (below), which is a design contribution independent of the run.
>
> **The relation-bearing-field correction is itself a patent-relevant design contribution (independent of the run).** During review, `charge_basis_components` (recording which bases were *present/co-mentioned* in a candidate's context) was found to permit a co-occurrence false match — a percentage taken against taxes/CAM, co-located with an operating-expense clause, could be certified as the operating-expense share, because the field recorded presence rather than *application*. The panel could classify every fact honestly and the policy would still certify the wrong value: a lossy-representation defect uncatchable by any downstream observation. The fix (v3.3) replaces it schema-wide with `value_applies_to_charge_basis_components` — *the bases the candidate's OWN value expression is applied to, established by cited value-to-basis linkage*; ungrounded linkage routes to undeterminable. This sharpens Guardrail #2 ("citation or it didn't happen") at the SELECTION layer: a citation must establish the value-to-basis *relationship*, not merely that the value and the category each occur. Generalizes the #26 evidence-capture governance to the selection field's semantics.
>
> **The review method is itself reduction-to-practice of the governance claim, and is dated evidence.** 431's package was gated by: a two-stage authorization (build with the call path structurally `NotImplementedError`; the live run separately sanctioned) so the ~105 calls cannot fire without an explicit token; a preregistered requirement profile independent of the fixtures; **blind adversarial review in isolated conversations** (prompt-leakage and profile-circularity, each denied the answer key and — for the profile review — denied any trace of the defect being tested, with the withholding recorded *before* the review in `build_log/431_blind_review_provenance.md`); and a **non-blind informed audit** holding the fixtures and history. The blind profile review INDEPENDENTLY found the co-occurrence defect (before the fix) and INDEPENDENTLY confirmed it closed (after), from a reviewer that never held the answer key — which is the evidentiary point of blinding. This blind-then-informed, provenance-recorded method is a concrete governance mechanism, dated and reduced to practice regardless of the run's outcome.
>
> **CLAIM BOUND (must survive to the report).** 431 measures GOVERNED SELECTION over these seven provisioned candidates — NOT general semantic accuracy, NOT recall, NOT production readiness, NOT share-vs-fee discrimination (a recorded generality/claim-scope debt: `basis_match` is a basis screen, and share-vs-fee identity is carried by the relevance/support/role conjunction, adequate for these fixtures but not a general share-discriminator). A `satisfied` result on the concept-substitution forcing case (cand_04) is a *grounded panel assertion* that the value applies to operating expenses — NOT proof it is legally correct, NOT validation that the document contains a genuine opex share. Grounded ≠ true; the mechanism governs whether a claim is cited and grounded, never whether it is correct.
>
> **=== END 431 STATUS NOTE. Supplement #26 (below) remains accurate; this note updates only the selector-panel line from DESIGNED to AUTHORIZED-NOT-YET-DEMONSTRATED-RTP. ===**
...
>
> **Status discipline (do not soften).** The defect is **fixed at the substrate level, not yet in the live pipeline.** Step 427 built and tested a named parameter block (`tenant_share`, `building_share`, `rent_adjustment_pct`, `base_rent`) with a declared dependency map (`LP-02`, `LP-07`) and deterministic, non-destructive attachment, gated by Gate B (`GateAbortError`, fail-closed, keyed to dependency names never literal values, never evaluator agreement). Run against the real Atreca document under `canonical_v2`: `LP-07`'s attached evidence context contains `tenant_share` (`"Tenant's Share of Operating Expenses of Building: 100%"`, offsets `[1942, 1996)`, verified) and `building_share` (`"Building's Share of Project: 45.79%"`, offsets `[1997, 2032)`, verified) — quoted, not characterized. **This is the first time in this project's record that the 100% has been mechanically attached to LP-07's evidence context.** It remains true, and must not be blurred, that **no live evaluator has yet seen it** — Step 427 is deliberately **not wired** into `lease_adapter.py`/`lease_coverage.py` (confirmed by seam tests), because Gate C (assignment stability across runs) has not been built or measured, and 423 spec §8 blocks Stage 5 work until Gates A–D pass together. **No performance baseline exists and none may be cited**; 417/419/420 remain void as system baselines. RTP: 414, 416, 421B, 422A–D, 423A/423B/423C (423C superseding 423B's LP-blind approach; 423B's module untouched, not deleted), 425's `canonical_v2` page-number strip (424→426 measurement: 92 of 166 unverified spans were parser artifacts, now zero; the exclusions list recovered 0/5→5/5, verified by direct provenance check after 424's own locator false-positive was caught and corrected), and now 427's parameter block + dependency map + Gate B (all built and tested, **not wired** into the live evaluator pipeline). DESIGNED-NOT-BUILT: selector panel, cited union, trace validation, structural addressing (see `Docs/Design_Note_Structural_Addressing_2026_07_14.md` — deliberately deferred; ellipsis-elision is 100% of what remains unverified post-425 and is a model behavior, not a parser problem). 334 tests passing.
>
